...
状态：Draft（**P-REFACTOR 已完成** 2025-04）  
配套：`v3-engine.md`、`v3-llm.md`、`v3-provider.md`、`v3-agent.md`、`v3-node-catalog.md`
...
P-REFACTOR  Provider+Agent 三层重构（已完成 2025-04）       架构升级
     - 新增 providers/ 模块（最底层凭据管理）
     - 新增 agents/ 模块（业务唯一入口；4 种 Runner：cline / simple_chat / react / external）
     - LLM 层降级为 Agent 内部依赖；ClineBackend 迁出
     - 三层物理隔离 + CI lint
     - 新 CLI：csim provider / csim agent；csim llm 标注调试入口
     - 详见 docs/rfc/v3-provider.md / v3-agent.md
...
## 4.5 P-REFACTOR：Provider + Agent 三层重构（已完成 2025-04）

### 目标

把 v3 从两层（LLM + Engine）改为**三层架构**：Provider → LLM → Agent → Engine → Node。
- **Provider 层（最底）**：管理「我有哪些 API 提供商账号 + 凭据 + endpoint」，唯一持有 raw api_key 的层
- **LLM 层（中）**：单次 chat / embed 调度（limiter / cache / audit）；通过 ProviderService 拿凭据
- **Agent 层（次顶）**：agent task 调度（limiter / cache / audit）；4 种 Runner（cline / simple_chat / react / external）
- **业务（顶）**：只见 AgentService；不许 import LLMService / ProviderService

### 验收

- 491+ 个 v3 测试全绿；五层 lint 0 violation
- `csim provider list / show / test`、`csim agent list / show / test / route / usage / audit / cache` 全部可用
- DashScope 真实 ping：`csim provider test dashscope_coding` ok
- 真实 cline 路径：`csim agent test --agent cline_real --spec data/agent_specs/director.toml`（dashscope coding qwen3.5-plus）跑通；audit 中 `runner_kind: cline`、`llm_calls_count: null`
- v2 的 57 个测试不受影响
...
| PR | 内容 | 体量 |
|---|---|---|
| R-1 Provider 层 | `providers/{types,errors,config,resolver,service,health}.py` + `providers.yaml` schema + `csim provider` + lint + 测试 | M |
| R-2 LLM 层重构 | `ModelDef.provider` 引用；`Resolver` 改从 `ProviderService` 取凭据；`base_url/api_key_ref/ollama_host` 删除 | S |
| R-3 Agent 框架 + Cline + SimpleChat | `agents/{types,errors,config,resolver,limiter,cache,audit,usage,service}.py` + `agents/runners/{base,cline,simple_chat}.py` + `agents.yaml` schema + `agent.run / agent.cline` 节点 + `EngineServices` 切换 + `csim agent` + 五层 lint | L |
| R-4 ReAct + External | `agents/runners/{react,react_tools,external}.py` + `_react_protocol.md` + 测试 | M |
| R-5 删除旧实现 | 删 `llm/backend/cline.py`；删 `LLMService._chat_backend_for` 中 Cline 特判；`llm.example.yaml` 重写 | S |
| R-6 文档 | 新 `v3-provider.md` / `v3-agent.md`；更新 `v3-llm.md` / `v3-engine.md` / `v3-implementation.md`（本节）/ `prompt-design-notes.md` | M |

### 完成标准（实际达成）

- 688 测试通过（491 + 后续补齐 197；含 12 个新测试文件覆盖 providers/agents/runners/cli/lint）
- agent_specs / 11 个 agent / 4 个 graphs / 3 个 subgraphs 全保留；P3 graph 通过 `agent.cline` alias 节点保持后向兼容
- golden expected scrub 清单扩展：新增 `agent_run_id / physical_agent / runner_kind / llm_calls_count / llm_ms`
- 真实 cline 路径（dashscope coding qwen3.5-plus）本地实跑：`text=READY`、`runner_kind=cline`、`exit=0`，74s 内完成 auth+task+工件回读
- **代理硬纪律**：所有 `httpx.AsyncClient(trust_env=False)` + 子进程 env 强制剥代理；`cfg.no_proxy` 开关已废弃（详见 `v3-agent.md §5.1` / `v3-provider.md §10.6`）

### 后续待办

- ReAct 工具集扩展（write_key / chroma_upsert）
- aider / codex 真实 CLI 集成（仅给框架 + 示例配置）
- Runner 热替换 / 多 agent 协作 / token cost 跟踪：留给后续阶段
