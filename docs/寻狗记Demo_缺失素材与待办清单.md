...
| 新场景深度/碰撞图 | 8 个新场景（雾津街头/灵堂/婆子家院/枯井/义庄/城隍庙夜/城门口/阎王岭山口） | 无遮挡关系、无墙体碰撞（仅大物件热区多边形）。用 `tools/character_lighting_lab` 逐场景烘焙生成（旧 `scene_depth_editor` 已删除） |
...
11. **背尸②崖墓内部命名改名 ✅ 已落地（2026-06-28）**：旧冥婚版残留内部 id 已全部转崖墓口径——scene id `庄家灵堂`→`崖墓`（场景 JSON 文件 + `resources/runtime/scenes/` 媒体目录同步改名）、zone `z_灵堂进场`→`z_崖墓进场`、信号 `lingtang_intro_entered`→`yamu_intro_entered`、scenario 状态 `at_lingtang`→`at_yamu`、热点 `T_出灵堂`→`T_出崖墓`、cutscene `寻狗_灵堂进场`→`寻狗_崖墓进场`、出口 label「去庄家（灵堂）」→「上崖墓」+ 画布/注释口径。跨 10 文件 + `XungouMainFlowIntegration.test.ts` 同步（其 `beishi_fled` 源也改为真实发出者 `寻狗_鬼打墙`）。验证：tsc 0 + 集成测试 14/14 + validate-data 0 error + 素材审计 0 issue 全绿。
12. **鬼打墙 真·空间循环 ✅ 已落地（2026-06-28）**：已从 PressureHold 长按升级为**玩家真走动的空间循环**——`寻狗_背尸` 在发力(carry PH)+香粉+不唱魂后**交控权**给玩家（`setFlag beishi_carrying`）；崖墓场景 `庄家灵堂` 加 `z_鬼打墙` 区 + `崖墓口` snap-back spawn，玩家往崖下走入区 → `startDialogueGraph 寻狗_鬼打墙`（staged by `guidaoqiang_loops`：绕回原地×2 各 `switchScene` 同场景 snap-back 崖墓口 + 第3次豁出去坠崖 → `triggerDeathTether` → `emit beishi_fled`）；走鬼打墙时 `hs_女尸`/`T_出灵堂` 隐藏（困住玩家、走不脱）。**保 `scenario_背尸` 信号契约**：try1/try2 仍由 `carry_bride_corpse` PH 发、scent 由对话发、fled 改由 `寻狗_鬼打墙` 发（黑盒声明已同步 dlg_02/dlg_guidaoqiang）。运行时实测：loop staging（`guidaoqiang_loops` 0→1、switch 命中 ==2）+ 坠崖 climax（系绳回血 15→60 + 清 `beishi_carrying`）全绿、0 console error。**已补润色（2026-06-28）**：snap-back 加了一瞬黑闪（`fadeWorldToBlack`/`fadeWorldFromBlack` 200/280ms）+ `sfx_guidaoqiang_loop` 阴风强化"绕回"错位感；旧 `guidaoqiang_escape` PressureHold 死条目已删除。
