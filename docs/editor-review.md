...
### DialogueBrowser 切换文件时静默保存

DialogueBrowser 的 `_load_ink_file` 方法在切换 Ink 文件时，如果当前文件有未保存的变更，会直接调用 `_do_save()` 写入磁盘，没有任何弹窗确认。用户修改了文件内容后只是想看看另一个文件的内容参考，变更已被静默写入。这与 CutsceneEditor 切换时弹窗询问 Save/Discard/Cancel 的行为不一致。
