...
ALL_DVC_TARGETS = [RUNTIME_TARGET, EDITOR_TARGET, VENDOR_TARGET, AUDIO_SOURCES_TARGET]
...
def init_audio() -> int:
    """把配音原始音源库拉到本机(tools/voice_workbench 的输入)。

    单独一个入口而不是并进 init_editor:70MB 素材只有工作台用,而工作台在
    ...
    print("Voice source library is ready.")
    return 0
...
def pull(editor: bool = False, audio: bool = False, git_proxy: str = "") -> int:
    ...
    配音音源走 --audio。音源之所以不并进 --editor(尽管工作台是从主编辑器菜单起的):
    scripts/pull-all.sh 无条件带 --editor,并进去就等于把 70MB 变成事实上的默认拉取,
    而绝大多数人拉仓库是为了跑游戏/改数据,不是为了重录配音。
    """
