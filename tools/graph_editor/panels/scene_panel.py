...
from tools.editor.shared.rich_text_field import RichTextLineEdit
...
class HotspotPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        iw.addWidget(QLabel("<b>inspect data</b>（无 graphId 时用 actions；与 graphId 互斥）"))
        mode_row = QHBoxLayout()
        self._insp_mode_grp = QButtonGroup(self)
        self._mode_actions = QRadioButton("Actions（无图对话）")
        self._mode_graph = QRadioButton("图对话 graphId")
        self._mode_actions.setChecked(True)
        self._insp_mode_grp.addButton(self._mode_actions)
        self._insp_mode_grp.addButton(self._mode_graph)
        mode_row.addWidget(self._mode_actions)
    ...
    def load_node(self, nd: NodeData):
        ...
        try:
            ...
            self._mode_actions.blockSignals(True)
            ...
            if gid:
                ...
                self._entry.clear()
            self._mode_actions.blockSignals(False)
