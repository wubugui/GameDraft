...
def _data_file(project_path: str | os.PathLike[str], name: str) -> str:
    """``public/assets/data/<name>`` 的绝对路径，统一走 ProjectPaths。"""
    return str(ProjectPaths(Path(project_path)).data_dir / name)


def _scenes_dir(project_path: str | os.PathLike[str]) -> str:
    return str(ProjectPaths(Path(project_path)).scenes_dir)
...
def parse_quest_groups(graph: GameGraph, project_path: str):
    fp = _data_file(project_path, "questGroups.json")
...
def parse_quests(graph: GameGraph, project_path: str):
    fp = _data_file(project_path, "quests.json")
    if not os.path.exists(fp):
        return
    with open(fp, "r", encoding="utf-8") as f:
        quests = json.load(f)
...
def parse_encounters(graph: GameGraph, project_path: str):
    fp = _data_file(project_path, "encounters.json")
    if not os.path.exists(fp):
        return
    with open(fp, "r", encoding="utf-8") as f:
...
def parse_items(graph: GameGraph, project_path: str):
    fp = _data_file(project_path, "items.json")
    if not os.path.exists(fp):
        return
    with open(fp, "r", encoding="utf-8") as f:
        items = json.load(f)
...
def parse_rules(graph: GameGraph, project_path: str):
    fp = _data_file(project_path, "rules.json")
    if not os.path.exists(fp):
        return
    with open(fp, "r", encoding="utf-8") as f:
        data = json.load(f)
...
def parse_scenes(graph: GameGraph, project_path: str):
    ...
    for fname in os.listdir(scenes_dir):
        ...
        _extract_flags_from_actions(graph, scene_id, scene.get("onEnter", []))
