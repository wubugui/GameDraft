...
**full-MIS,无近似**:BSDF 侧对**每一根命中射线**按 pdf_light 降权,不按
命中像素过滤。历史教训(§15,对抗审查 N-1):曾只对发光像素注册命中降权并
自称「有界小偏差(pl ≲1e-3)」—— 实测是阴影区单向 **+15%** 漏光(被挡方向
p_L/p_B 中位 4.74、MIS 总权重至 1.97),且两个 8% 能量门的几何恰好对这一类
失明。full-MIS 下被挡方向的挡板辐射由 w_B·L + w_L·L = L 严格分账,
逐向权重归一无条件成立;钉子测试 = 被完全遮挡发光体的零能量注入。
