...
def load_last_run_path() -> Path | None:
    ...
    if not p.is_dir():
        return None
    # v2：run.json；旧版 chronicle_sim 可能仍有 run.db
...
def save_main_splitter_state(state: QByteArray) -> None:
    _settings().setValue("ui/main_splitter", state)


def load_main_splitter_state() -> QByteArray | None:
    v = _settings().value("ui/main_splitter")
    if v is None or v == "":
        return None
    if isinstance(v, QByteArray):
        return v
