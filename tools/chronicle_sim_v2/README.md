...
- **项目本地 Python**：使用仓库内 `.\.tools\Python311\python.exe`，不要使用全局 Python
...
set PYTHONPATH=%CD%
.\.tools\Python311\python.exe tools\chronicle_sim_v2\scripts\run_simulation_once.py tools\chronicle_sim_v2\runs\<run_id> --week 1
.\.tools\Python311\python.exe tools\chronicle_sim_v2\scripts\run_simulation_once.py tools\chronicle_sim_v2\runs\<run_id> --from 1 --to 3
```
...
- **Cline 任务正文**：runner 始终将完整 user 写入临时 cwd 的 ``input.md``；``cline task`` 的 argv 末参**固定**为短引导句（要求先 ``read_file`` ``input.md``），**不把**长 prompt 拼进命令行，避免 Windows ``CreateProcess``「命令行太长」导致 ``rc=1``（stderr 在部分控制台会显示为乱码）。
...
单独校验谣言传播（默认临时最小世界 + stub）::
    ...
    .\\.tools\\Python311\\python.exe tools\\chronicle_sim_v2\\scripts\\run_rumor_spread_standalone.py
    .\\.tools\\Python311\\python.exe tools\\chronicle_sim_v2\\scripts\\run_rumor_spread_standalone.py --run-dir <已有 run 目录>

对已有 Run 的某一周做**统计**（复制 ``world``、**原样**使用 Run 的 ``rumor_sim``；默认 ``run_rumor_spread(..., skip_distort_llm=True)`` 不调走样 LLM）::

    .\\.tools\\Python311\\python.exe tools\\chronicle_sim_v2\\scripts\\run_rumor_week_stats.py --run-dir <run 目录> --week 1

若要在统计脚本里真的走 ``_distort_one``（stub 占位或 Cline，取决于 ``pa``）::

    .\\.tools\\Python311\\python.exe tools\\chronicle_sim_v2\\scripts\\run_rumor_week_stats.py --run-dir <run 目录> --week 1 --call-distort-llm

只读已落盘的 ``rumors.json``，统计 ``distorted: true`` 条数::

    .\\.tools\\Python311\\python.exe tools\\chronicle_sim_v2\\scripts\\run_rumor_week_stats.py --run-dir <run 目录> --week 1 --from-disk-rumors
...
.\.tools\Python311\python.exe -m pytest tools\chronicle_sim_v2\tests -q
```
