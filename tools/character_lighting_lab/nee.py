"""NEE(下一事件估计)= 第二个**方向采样器** + 立体角 MIS。

从 lighting-rebuild 分支 `tools/lightbake/nee.py` 搬运(2026-09-01,制作人定位
「probe 高分位不收敛」后补课)。改动仅两处:trace 无 max_distance(本库射线无限,
制作人铁律),LUMA 本地定义。数学与分支逐字一致,注释保留原文要点:

伪世界的发光体 = HDR 展开后辐亮度超过 `NEE_EMITTER_MIN` 的画面像素(灯芯/窗光,
实测 ~200-1200;普通反射面 <1)。对每个 gather 样本追加一根**朝发光体的方向样本**,
march 打到什么就取什么的辐射 —— 可见性完全留在被积函数里,由唯一 tracer 定义。
与余弦/均匀球(BSDF)样本用 balance heuristic 合并(Veach 1995),组合无偏。

## 发光元的几何 = 壳体素箱

march 在 `pen ∈ (bias, thick)` 记命中 ⇒ texel 在 march 测度下的真实几何是
**像素列 × z 窗的体素箱**(`trace.SHELL_WINDOW`)。密度 = 沿射线对**所有**穿过的
发光箱求和(DDA 精确弦积分):

    pdf_L(w) = Σ_{j: 射线∩箱_j ≠ ∅} p_sel_j · (t_out³ − t_in³) / (3·V_box)

厚壳箱在射线方向上连排相交,单箱口径会把密度低估「多重覆盖」倍 ⇒ f/pdf 超收
(分支合成场实测 ×3.6)。**full-MIS,无近似**:BSDF 侧对每一根命中射线按
pdf_light 降权,不按命中像素过滤(分支验尸:sel_map 过滤 = 阴影区单向 +15% 漏光)。

## 事件类

- f_hit:两策略 MIS 分账(光源样本取 march 命中处辐射,逃逸 = 合法零样本);
- f_esc(天空/逃逸辐射):只归 BSDF —— 光源策略不覆盖天空方向,单策略全权无偏。

随机流:位置哈希 + 独立盐(sampling._SALT_NEE,4 条流) —— 字节可复现。
"""
...
__all__ = ['NeeContext', 'build_nee', 'pdf_light', 'sample_light', 'LUMA']
...
class NeeContext:
    """一张画的发光体表。构建一次,probe gather 与逐像素 ref 共用(辐亮度不进表:
    消费者从**自己的**辐射图按 march 命中像素取;p_sel 只是选取 pdf)。"""
...
def build_nee(hdr: np.ndarray, field: DepthField,
              threshold: float | None = None) -> NeeContext | None:
    """从展开后的 HDR 画面建发光体表;没有阈上发光体返回 None ——
    调用方拿 None 就走纯 BSDF 老路,逐位不变。

     缺省 NEE_EMITTER_MIN。⚠ 分支的 4.0 是它 to_hdr 标度
    (发光体 200-1200)下定的;本管线恢复上限 maxEV=5(全场最亮 ~30),
    同一个 4.0 会把中亮像素全漏给 BSDF —— 调用方按自己的辐射标度传。"""
    thr = float(NEE_EMITTER_MIN if threshold is None else threshold)
...
def pdf_light(ctx: NeeContext, origins_q: np.ndarray,
    ...
    分母与 BSDF 命中的 MIS 权都从这里拿(类边界两侧口径逐位一致)。"""
...
def sample_light(ctx: NeeContext, origins_q: np.ndarray, keys: np.ndarray,
    ...
    壳箱内三维均匀抖动;pdf 经 `pdf_light`(DDA 池化)重求值。"""
