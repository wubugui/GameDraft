...
class MainWindow(QWidget):
    ...
    def _workspace_default_dir(self) -> str:
        """工作区目录默认从 ``resources/editor_projects/editor_data/animation`` 起浏览。"""
        from tools.editor.shared.project_paths import (
            DIR_KIND_EDITOR_ANIMATION_PROJECT,
            ProjectPaths,
        )
        repo_root = Path(__file__).resolve().parents[2]
        d = ProjectPaths(repo_root).default_dir_existing_or_root(
            DIR_KIND_EDITOR_ANIMATION_PROJECT,
        )
        return str(d)
    ...
    def _open_workspace(self) -> None:
        d = QFileDialog.getExistingDirectory(
            self, "打开工作区目录", self._workspace_default_dir(),
        )
        if not d:
            return
        try:
    ...
    def _save_workspace_as(self) -> None:
        d = QFileDialog.getExistingDirectory(
            self, "选择保存目录", self._workspace_default_dir(),
        )
        if not d:
            return
        try:
