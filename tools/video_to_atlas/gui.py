...
class MainWindow(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        left_panel = QWidget()
        left_flow = QVBoxLayout(left_panel)
        left_flow.setContentsMargins(0, 0, 0, 0)
        ...
        self.edit_sprite_path = QLineEdit("/resources/runtime/images/characters/out_atlas.png")
        ...
        left_flow.addWidget(gb_out)

        self.lbl_status = QLabel("")
        self.lbl_status.setWordWrap(True)
        left_flow.addWidget(self.lbl_status)
        left_flow.addStretch(1)
        ...
        rl.addStretch(1)
        self.lbl_main_hint = QLabel(
            "右侧为源视频；左侧为单页工作流：视频/抽取 → 色键 → 图集读入与批处理 → 帧库 → 动画 → 导出。"
        )
