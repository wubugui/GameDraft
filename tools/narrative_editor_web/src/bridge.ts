...
type QtBridge = {
  ...
  getRefactorJournalSize?: (cb: (result: string) => void) => void;
};
...
declare global {
  interface Window {
     ...
     * **不删除**，供顶层 ErrorBoundary 崩溃页导出草稿、宿主壳兜底读取（P1-07）。
     */
    __narrativeEditorLastDraft?: string;
    /** 顶层 ErrorBoundary 捕获到渲染崩溃后置 true，供宿主壳区分「崩溃」与「加载中」。 */
    __narrativeEditorCrashed?: boolean;
  }
}
...
export async function scanSignalUsagesRemote(
  ...
  // 带上当前画布草稿：预览计数按「画布现状」算而非模型旧值（宿主对老格式裸 id 仍兼容）。
  const arg = draft ? JSON.stringify({ signalId, data: draft }) : signalId;
  return new Promise((resolve) => {
    bridge.scanSignalUsages!(arg, (payload) => {
      try {
...
export async function scanStateUsagesRemote(
  ...
  const arg = draft ? JSON.stringify({ graphId, stateId, data: draft }) : graphId;
  return new Promise((resolve) => {
    bridge.scanStateUsages!(arg, stateId, (payload) => {
      try {
...
export async function scanGraphUsagesRemote(
  ...
  const arg = draft ? JSON.stringify({ graphId, data: draft }) : graphId;
  return new Promise((resolve) => {
    bridge.scanGraphUsages!(arg, (payload) => {
      try {
...
/**
 * 查询宿主侧重构撤销日志长度：页面（重）加载后据此恢复「撤销重构(n)」按钮——
 * journal 挂在 ProjectModel 上、跨页面加载存活，此前 web 重载即丢入口（P3）。
 */
export async function refactorJournalSizeRemote(): Promise<number> {
  const bridge = await waitForBridge();
  if (!bridge?.getRefactorJournalSize) return 0;
  return new Promise((resolve) => {
    bridge.getRefactorJournalSize!((payload) => {
      try {
        const parsed = JSON.parse(payload) as { size?: number };
