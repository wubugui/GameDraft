/**
 * RHI 冒烟页入口(WebGPU)。`?case=关键字` 只跑部分用例;跑完把结果挂在 `window.__rhiSmoke`
 * (无头驱动读它,见 run.mjs),同时画成表格给人看。
 */
import { createRhiDevice, RhiError } from '@src/rendering/rhi';
...
const report: SmokeReport = { done: false, renderer: '', fatal: null, results: [] };
...
function render(): void {
  ...
  rows.replaceChildren(
    ...report.results.map((r) => {
      ...
      const mark = { pass: '✓ 通过', fail: '✗ 失败' }[r.status];
  ...
  const head = report.fatal
    ? `设备创建失败:${report.fatal}`
    : `WebGPU · ${report.renderer} —— 通过 ${pass} / 失败 ${fail}${report.done ? '' : ' · 运行中…'}`;
...
async function main(): Promise<void> {
  ...
  let dev;
  try {
    dev = await createRhiDevice({ canvas, useDevicePixels: false, autoResize: false });
  } catch (e) {
  ...
  report.renderer = dev.info.renderer || dev.info.vendor;
  render();
