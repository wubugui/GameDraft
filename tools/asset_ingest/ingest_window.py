...
_CATEGORIES: list[_Category] = [
    _Category("游戏 / 背景", "public/resources/runtime/images/backgrounds", "一般留空", False),
    _Category("游戏 / 立绘", "public/resources/runtime/images/illustrations", "可选子目录，如 埃德加、道具", True),
    _Category("游戏 / 角色头像", "public/resources/runtime/images/characters", "一般留空", False),
    _Category("游戏 / 音频", "public/resources/runtime/audio", "一般留空", False),
    _Category("工作区 / 动画原视频", "resources/editor_projects/editor_data/animvideo", "一般留空", False),
    _Category("工作区 / 未分类收件箱", "resources/editor_projects/editor_data/asset_inbox", "先扔这里再二次整理", False),
]
...
def _is_safe_custom_relpath(s: str) -> bool:
    """允许迁移后两类目标根：``public/...`` 或 ``resources/editor_projects/editor_data/...``。

    与 :class:`ProjectPaths` 的根定义保持一致。
    """
    s = s.replace("\\", "/").strip().strip("/")
    if not s:
        return False
    low = s.lower()
    return low.startswith("public/") or low.startswith("resources/editor_projects/editor_data/")
...
class IngestWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._custom.setPlaceholderText("例如 public/resources/runtime/images/illustrations/新主题")
        self._custom.setToolTip("必须以 public/ 或 resources/editor_projects/editor_data/ 开头，相对仓库根。")
    ...
    def _append_log_file(self, entries: list[dict]) -> None:
        log_path = self._root / "resources/editor_projects/editor_data" / "asset_ingest_log.json"
    ...
    def _do_import(self) -> None:
        ...
        if dest is None:
            QMessageBox.warning(
                self,
                "无法导入",
                "请选择有效分类，或填写以 public/ 或 resources/editor_projects/editor_data/ 开头的自定义相对路径。",
            )
            return
        try:
        ...
        if n_ok == len(srcs):
            QMessageBox.information(
                self,
                "完成",
                f"已处理 {n_ok} 个文件。记录已追加到 resources/editor_projects/editor_data/asset_ingest_log.json。",
            )
        elif n_ok > 0:
            QMessageBox.warning(
                self,
