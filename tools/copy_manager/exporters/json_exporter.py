...
class JsonExporter:
    ...
    def __init__(self, project_root: Path):
        ...
        self._strings_rel = self._paths.disk_to_assets_url(
            self._paths.data_dir / "strings.json",
        ) or "/assets/data/strings.json"
        # 兼容历史 file_path 字符串（去掉前导斜杠并加 public 前缀），
        # entries 里 file_path 是相对仓库根的 posix 路径，例如 "public/assets/data/strings.json"。
        self._strings_file_key = "public/assets/data/strings.json"
    ...
    def export(
        ...
        if self._strings_file_key in by_file:
            out_path = self._export_strings(
                by_file.pop(self._strings_file_key), language
            )
            if out_path:
    ...
    def _export_strings(self, entries: list[dict], language: str) -> Path | None:
        ...
        src = self._paths.data_dir / "strings.json"
        if not src.exists():
            return None
        ...
        out = self._output_path(self._strings_file_key, language)
        write_json(out, data)
        return out
    ...
    def _output_path(self, original_path: str, language: str) -> Path:
        """Compute the output path for a translated file under data/<lang>/<filename>。"""
        # 与扫描器一致：``original_path`` 以 ``public/assets/data/`` 开头
        prefix = "public/assets/data/"
        if original_path.startswith(prefix):
            filename = original_path[len(prefix):]
        else:
            filename = original_path.split("/")[-1]
        return self._paths.data_dir / language / filename
