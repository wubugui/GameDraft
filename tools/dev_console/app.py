...
TOOLS: tuple[ToolAction, ...] = (
    ...
    ToolAction("呼吸工作台", "breathing-workbench",
               "呼吸图(盖脸纸那类「一张静帧实时在呼吸」的叠图):节奏 / 胸口 / 纸 / 纸和胸口怎么对上 / 猛吸 / 渐弱全部参数实时调,"
               "页面里跑的就是游戏那份模拟与着色,能按对话图里那一段剧情原样走一遍、出循环 GIF 与剧情 MP4;"
               "推给游戏(拖参数时跑着的游戏里跟着变,资源不动)→ 导出到游戏(写进资源);"
               "它是 assets/data/breathing/ 的唯一写者(独立窗口,零浏览器缓存)"),
