...
_AGENT_ENTRIES: list[tuple[str, str]] = [
    ...
    ("tier_a_npc", "NPC Tier A+B（A 与龙套 B 共用本页）"),
    ("gm", "GM 世界"),
    ("director", "编年史导演"),
    ("rumor", "谣言"),
    ("week_summarizer", "周总结"),
...
KNOWN_TOP_LEVEL = frozenset(
    {"embeddings", "semantic_memory", "llm_audit", *(k for k, _ in _AGENT_ENTRIES)},
)
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        for key, label in _AGENT_ENTRIES:
            ...
            if key == "tier_s_npc":
                ...
                item.setToolTip(
                    "current_tier=A 与 current_tier=B 的 NPC 均使用本页同一套提供方与模型（配置键 tier_a_npc）。\n"
                    "程序内不设 tier_b_npc；B 与 A 的差别只在系统提示词（B 使用 data/prompts/npc_tier_b.md，篇幅更短）。"
                )
