...
已有实例在跑时把 ``open:<id>`` 送过命名管道,已开着的窗口切到那张呼吸图)。

``--selftest``:**整个进程的读写都指到临时样例工程**(``fixtures.build_project``)——自检只在那里存盘、出片,
    ...
    def _on_activate(data: bytes, view) -> None:
        if data.startswith(b"open:"):
            ...
            if bid and view is not None:
                view.page().runJavaScript(f"window.__openBreathing && window.__openBreathing({json.dumps(bid)})")
