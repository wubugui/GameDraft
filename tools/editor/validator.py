    ...
    *,
    ...
    if raw is not None:
        if not isinstance(raw, (str, dict)):
            ...
            elif voice_id not in (model.audio_config.get("sfx") or {}):
                issues.append(Issue(
                    "warning", data_type, item_id,
                    f"{label} voice {voice_id!r} 不在 audio_config.sfx 中",
                ))
            if isinstance(raw, dict):
                vol = raw.get("volume")
    ...
    for i, step in enumerate(steps):
        ...
        if kind == "action":
            ...
            if t in ("showSubtitle", "showDialogue"):
                ...
                own_voice = bool(_voice_spec_id(_voice_raw_of(
                    step, legacy_key="subtitleVoice" if t == "showSubtitle" else None)))
                if held:
                    voice_sustain[0] = True
                elif own_voice:
                    voice_sustain[0] = False  # 起了新配音又不留声：本拍结束即停
