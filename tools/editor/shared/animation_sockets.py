"""动画挂点（sockets）、落脚帧（contactSlots）与点火接触帧（igniteSlots）的编辑器侧读写与几何。
...
点火接触帧（``igniteSlots``，燃烧系统 A3.8）与落脚帧**完全同口径**（槽位、清洗、缺省不落键、
同一个面板勾）；运行时点火表演取点火片段帧序列里第一个落在它上面的帧作为火头碰到可燃物那一刻。
...
#: 点火接触帧：点火动作里火头碰到可燃物的图集槽位（升序去重），与落脚帧同口径同住 sidecar。
#: 运行时 `SpriteEntity.igniteContactFrame` 取点火片段帧序列里**第一个**落在这里的帧；
#: 空 / stale = 没标，运行时退到片段第一帧（dev 告警一次）。
IGNITE_SLOTS_KEY = "igniteSlots"
...
def save_socket_set(path: Path, data: dict) -> Path:
    ...
    **整份为空时删文件**：既没有挂点、也没有落脚帧、也没有点火接触帧，就不该在包目录里
    留一个空壳，否则每个动画包都多一个永远是 `{}` 的文件。只标了落脚帧（或只标了点火接触帧）、
    一个挂点都没有的 sidecar 是合法且常见的（绝大多数会走路的包都这样），必须保留。
    """
    p = Path(path)
    sockets = data.get("sockets")
    has_sockets = isinstance(sockets, dict) and bool(sockets)
    if not has_sockets and not contact_slots_of(data) and not ignite_slots_of(data):
        if p.is_file():
            p.unlink()
        return p
...
def _set_slot_mark(data: dict, key: str, slot: int, on: bool) -> bool:
    """槽位布尔标注（落脚帧 / 点火接触帧共用）：就地改 data，清空即删键。返回是否真的变了。"""
    cur = set(normalize_contact_slots(data.get(key)))
    s = int(slot)
    if on:
        if s in cur:
            return False
...
def set_contact_slot(data: dict, slot: int, on: bool) -> bool:
    ...
    return _set_slot_mark(data, CONTACT_SLOTS_KEY, slot, on)


def normalize_ignite_slots(raw: object, slot_count: int = 0) -> list[int]:
    """点火接触帧槽位规整——与落脚帧**同一个口径**（TS 侧两者都走 `parseContactSlots`）。"""
    return normalize_contact_slots(raw, slot_count)


def ignite_slots_of(data: dict | None) -> list[int]:
    """读一份（内存态或磁盘态）挂点集里的点火接触帧槽位；缺省/坏形状 = 空。"""
    if not isinstance(data, dict):
        return []
    return normalize_ignite_slots(data.get(IGNITE_SLOTS_KEY))


def set_ignite_slot(data: dict, slot: int, on: bool) -> bool:
    """把某个图集槽位标成/取消点火接触帧（就地改 data）。返回是否真的变了。"""
    return _set_slot_mark(data, IGNITE_SLOTS_KEY, slot, on)
...
def sanitize_socket_set(data: dict, anim: dict) -> dict[str, Any]:
    ...
    # 点火接触帧：与落脚帧同一套（非空才落键、按当前图集裁上界），键序紧跟 contactSlots
    ignite = normalize_ignite_slots(
        data.get(IGNITE_SLOTS_KEY) if isinstance(data, dict) else None,
        int(out["atlas"].get("slotCount") or 0),
    )
    if ignite:
        out[IGNITE_SLOTS_KEY] = ignite
