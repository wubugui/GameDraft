...
def build_id_sets(model: ProjectModel) -> dict[str, set[str]]:
    ...
    rhyme_entries = model.archive_rhymes
    if isinstance(rhyme_entries, dict):
        rhyme_entries = rhyme_entries.get("entries", [])
    rhyme_ids = {e["id"] for e in rhyme_entries if isinstance(e, dict) and "id" in e}
    # 线索（K7）：clue_<id> 动态 flag 的 id 面。读 ref_validator 的同一通道（装载工程后
    # 走 ProjectModel.clues_registry 活数据；懒 import 防 project_model→flag_registry→ref_validator 环）。
    ...
    return {
        ...
        "archive_rhyme": rhyme_ids,
        "clue": clue_id_set(model),
    }
