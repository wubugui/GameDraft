/**
 * 圆角多边形路径(弧 / 二次曲线两种圆角)。移植自 PixiJS v8.17(MIT):scene/graphics/shared/path/roundShape。
 */
...
export function roundedShapeArc(g: ShapePath, points: RoundedPoint[], radius: number): void {
  const vecFrom = (p: PointData, pp: PointData): { len: number; nx: number; ny: number } => {
    const x = pp.x - p.x;
    const y = pp.y - p.y;
  ...
  const sharpCorner = (i: number, p: PointData): void => {
    if (i === 0) {
      g.moveTo(p.x, p.y);
    } else {
      g.lineTo(p.x, p.y);
...
export function roundedShapeQuadraticCurve(g: ShapePath, points: RoundedPoint[], radius: number, smoothness?: number): void {
  const distance = (p1: PointData, p2: PointData): number => Math.sqrt((p1.x - p2.x) ** 2 + (p1.y - p2.y) ** 2);
  const pointLerp = (p1: PointData, p2: PointData, t: number): PointData => ({
    x: p1.x + (p2.x - p1.x) * t,
    y: p1.y + (p2.y - p1.y) * t,
  });
