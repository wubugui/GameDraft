...
/** 天光。经**烘出来的传输基**调制（`lighting3/transport.png` 的 4 个纬向通道）。 */
export interface SkyLightDef {
  kelvin?: number;
  color?: RgbColor;
  intensity: number;
  /**
   * 天穹辐亮度的**纬向分布** 0..4：`L(ω) ∝ y^profile`（y = 方向的向上分量）。
   *
   * - `0` 均匀阴天（各向同性）
   * - `1` 余弦天穹（最常见的多云/一般天空）
   * - `2` / `4` 越来越集中于天顶（晴朗正午）
   *
   * 中间值在通道间线性插值，与烘焙侧 `interpolate_transport` 逐段同式。
   *
   * ⚠ 这个参数**取代了 v2 的 `hemi`**，两者不是一回事：`hemi` 是
   * `(1−h)+h·V` 里那个凑出来的混合权重，没有量纲意义，且和 `day.hemi` 一起
   * 把天穹遮蔽整个约掉过（实测 28/28 场景背景对遮蔽零响应）。
   * `profile` 描述的是**天空本身长什么样**，遮蔽与朝向已经全在传输基里。
   */
  profile: number;
}
 ...
 * ## 为什么必须有这一项
 *
 * 天光项 `I·T` 在完全遮蔽处归零，但真实场景里光会绕进来。烘焙期反解 `E_est`
 * 时这一项是显式拟合出来的（`E_est = c₀ + c₁·T₀`，实测 c₀ 在 0.44–1.38 之间，
 * **多数场景里它比天光项还大**）。目标环境如果只有 `I·T` 而没有底，
 * 巷道与檐下会黑得不合理 —— 那不是"打光风格"，是漏了一项。
 *
 * ⚠ 它**吃遮蔽但不归零**：`(0.28 + 0.72·T₀)`，与烘焙侧同式。全黑的角落仍留 28%，
 * 因为多次反弹本来就能绕进去；用纯常数则会让已经被遮蔽的区域被二次压暗。
 */
...
// ⚠ v2 的 `DayReferenceDef`（`S_day`）已整体移除。
// 「除掉原画里的白天光」现在**在烘焙期做完**：`tools/scene_relight/bake_gbuffer.py`
// 在伪世界里做 final gather 积出辐照度 `E`，把画拆成 `base·E + emissive`
// （`lighting3/base.png` + `irradiance.png`，这条是恒等式；载荷里没有自发光）。
// 运行时只做 `base × E_目标 + emissive`，没有第二个分母，也就没有 `day.hemi`
// 这种「必须与原画匹配、手填必错」的运行时参数了。
...
export interface FogDef {
   ...
   * 散射强度（雾自身的**辐射**）。
   *
   * ⚠ v3 起量程变了：雾是混进**线性 HDR 辐射场**的（`lin·T + scatter·(1−T)`），
   * 而那个场经 `to_hdr` 展开后能到 200，不再是 v2 那种落在 [0,1] 的画面值。
   * 想让远景雾看起来是"中灰"，这里要填的不是 0.5 —— 显示端的 reinhard 会把它
   * 压回去，`x/(1+x) = 0.5` 解出来是 **1.0**。当前没有场景开雾（sigma 全为 0），
   * 所以没有迁移问题；第一个开雾的人按新量程调即可。
   */
  scatter: number;
}
...
export interface SceneLightingDef {
  // ⚠ v2 的 `placeholder` 已移除。它是"恒等占位"标记，而恒等只对**背景**成立
  //   ——所以 `Game.setupUnifiedCharacterLighting` 见到它就把角色那一路整个
  //   `teardown()` 回落旧 probe。后果是 27 个场景里**你摆多少灯角色都零响应**，
  //   而背景照常亮，面板还不给任何提示。
  //
  //   v3 不需要它：烘焙期在伪世界里 final gather 积出原画自身的辐照度 `E`，
  //   作为**烘焙 GI** 随包发出。`gi = 1` 时 `base·E + emissive ≡ 原画`
  //   （精确到量化），未调过的场景画面因此就是原画 ——**用光照方程里一个有
  //   物理含义的项达成，而不是用一个把角色挡在门外的开关**。
  /**
  ...
  sky: SkyLightDef;
  /**
   * 环境反弹底。缺省 `{ intensity: 0 }`（无底 —— 只在你确实想要纯天光时这么填）。
   *
   * ⚠ 缺省 0：v3 的默认路径是 `gi = 1`（吃烘焙 GI），天光/环境是**重打光时**
   * 才加上去的项。`lighting3/meta.json` 的 `runtime_fit` 给了一组建议值 ——
   * 那是"若要用解析天光+环境替换掉烘焙 GI，大约该填多少"，供作者起步用。
   */
  ambient?: AmbientLightDef;
  /**
   * **烘焙 GI 的权重**。缺省 1。
   *
   * `lighting3/irradiance.png` 是烘焙期在伪世界里 final gather 积出的
   * **原画自身的辐照度**（射线打到表面就取那里的 HDR 原画辐射，逃逸就取天空
   * 辐射 —— 画本身就是辐射缓存）。`gi = 1` ⇒ 画面精确等于原画。
   *
   * 重打光就是把它调低、把 `sky` / `lights` 加上去。这是一条**连续**的路：
   * 0.6 的 gi 配上一点天光，就是"保留原画的大部分氛围但换个时刻"。
   *
   * ⚠ 别把它当开关用。它是光照方程里的一项，不是"要么原画要么全新"。
   */
  gi?: number;
  /**
   * **角色**吃多少烘焙 GI。缺省**跟随场景的 `gi`**。
   *
   * ⚠ 这不是被删掉的那个屏幕空间反弹 pass（`GiBouncePass`，已删）。它是
   * 烘焙期在伪世界里 final gather 积出的**场景自身辐照度**，角色按自己的世界
   * 位置与法线从 `sky_sh_grid.bin` 的 GI 通道里三线性查出来 —— 这正是
   * "角色要融入场景"要的东西。
   *
   * ⚠ 缺省跟随 `gi` 而不是恒 1 —— 恒 1 的话作者把 `gi` 调低重打光时背景暗了、
   * 角色纹丝不动（真机验过）。只在**故意**要角色与背景吃得不一样多时才显式填。
   *
   * ⚠ 调到 0 之前先确认场景有别的光。未重打光的场景 `sky`/`lights` 都是 0，
   * 此时 `charGi = 0` 会让角色**全黑** —— 那正是 v2 `placeholder` 造成过的症状。
   */
  charGi?: number;
  /**
   * **烘焙自发光**（`max(I − base·E, 0)`：灶口、灯笼、天）的整体缩放。缺省 1。
   *
   * 换天、把灯吹灭这类改动改这里；它不乘任何 `E`，因为它不反射光，它就是光。
   *
   * ⚠ 与下面的 `emissive` **不是一回事**，别看混：
   * · 本项 = 原画里**本来就在发光**的东西（烘焙期按定义拆出来的）
   * · `emissive` = 运行时给**手摆的灯**画一个可见灯体（gain / radius）
   */
  lights: LightDef[];
  fog?: FogDef;
  display: DisplayTransformDef;
  /**
   * 角色图集的**参考天穹强度** —— 把角色送进和场景同一个等价 albedo 空间的那一步。
   *
   * 角色图集也是美术在某种隐含光照下画的，所以它同样要除掉自己的 `E`：
   *
   *     A_角色 = 图集线性值 / ( (1 + N·up)/2 × charRefIntensity )
   *
   * 括号里正是**无遮挡、均匀阴天**下的传输（与场景侧同一个归一化约定：
   * 朝上面 1、竖直面 0.5），所以两边出来的 albedo 是同一个量纲。
   * 含义直白：「美术把角色当成被多强的天穹照亮来画的」。缺省 1。
   *
   * ⚠ 这一项**取代了 v2 的 `radianceScale`**。那个是拿"图集平均亮度 ÷ 场景反解
   * 反射率"凑的**标量**，补不上一个逐像素的场；这里是逐像素解析除。
   */
   ...
   */
  giGain?: number;
  // ⚠ v2 的 `dehaze` / `aoStrength` / `ratioMax` 已在 v3 移除，别再加回来：
  //   · 去霾现在在**烘焙期**做完（结果烤进等价 albedo），运行时无从也无需再去一遍；
  //   · `aoStrength` 是"要不要吃遮蔽"的可读性旋钮 —— 遮蔽已在传输基里，
  //     调它等于让画面偏离几何，属于拿参数掩盖模型问题；
  //   · `ratioMax` 是"原画 × 比值"那种写法才需要的护栏。albedo 显式之后，
  //     乘出来多少就是多少，钳位只会把标定错误藏起来（要看越界用 F2 的
  //     OVERBRIGHT 视图，那是**指出**问题而不是掩盖）。
  /**
...
export interface NpcDef {
   ...
   *
   * ⚠ 「不附加逐 entity 光照」**不等于不受重打光影响**（2026-08-23 起）：
   *
   * 这类实体挂的是 `RawPatchRelightFilter` —— 只吃 `E_目标/E` 这个比值，
   * 不做基底反解。于是既不会与背景色调不符（那是叠角色着色才会有的毛病），
   * 又能跟着作者调 `gi` / 加灯一起变。
   *
   * 两条性质都是真机验过的：
   * · `gi = 1` 时开关该滤镜画面**差值逐项为 0**（比值恒等于 1，逐像素不变）；
   * · `gi` 1.0→0.15 时背景 42.6→16.1，补丁 56.3→20.0 跟着走
   *   （不挂滤镜的话补丁停在 55.4，会明显浮起来）。
   */
  renderRaw?: boolean;
  /**
...
export type EmoteBubbleOffsetOpts = {
  ...
  scale?: number;
  /**
   * 「墨匣」皮肤分型（2026-08-18 气泡重设计定稿）：speech=剧情对白（满档，缺省）；
   * chatter=氛围碎嘴（底/勾线/字各弱一档，环境音不与剧情话同权重）；
 ...
 * `showDialogue` / `showSubtitle` 另可选用 `typewriter`（真布尔）：本拍逐字显示与否。
 * **缺省按台词面分家**——对白框逐字、字幕整句上屏；只在偏离缺省时写这个键。
 * 玩家在设置页关掉逐字则一律整句（玩家偏好压过编排）；逐字期间点击**只补完当前这句**，
 * 补完后再点才推进本拍。
...
export interface DayPhaseDef {
  ...
  label?: string;
  /**
   * **语义角色：这一段「人在外面做事」。** 未写 `phases` 的 NPC（龙套/群演）
   * 就只在打了这个标记的段里出现，见 `dayTime.daylightPhaseIds`。
   *
   * 存在的理由：**代码不许记住时段 id**。`id` 是内容侧的设定（辰/午/暮/夜，
   * 想改名、拆段、加段都随意），代码只认这个角色标记，于是词表怎么换都对不上不了。
   * 2026-08-18 之前这里是代码常量 `['day']`，内容侧一换词表就整条街空了还不报错。
   *
   * 一段都没标 = NPC 缺省不施加限制（全时段都在）并告警——宁可多几个人，绝不静默清空。
   */
  daylight?: boolean;
}
...
/**
 * 可独立调音量的音频通道。**对白单独一条不是设计洁癖**:玩家把音效压低时
 * 台词必须还听得见(听障/外语玩家更依赖它),这是有对白的游戏的通行做法。
 */
