...
export interface SugarWheelInstance {
  ...
  /**
   * 按下蓄力钮时对单条 `ConditionExpr` 求值（与热区等同源，见 `evaluateConditionExpr`）。
   * 缺省或省略视为通过。通过则立刻进入蓄力；松手确认发射前顺序执行 `beforeChargePassActions`。
   * 不通过则执行 `beforeChargeFailActions` 且不蓄力。
   */
  beforeChargeCondition?: ConditionExpr;
  /** 条件通过并松手确认发射前顺序执行（可为空），常用于扣除抽奖消耗。 */
  beforeChargePassActions?: ActionDef[];
  /** 条件不通过时顺序执行，且不进入蓄力（可为空）。 */
  beforeChargeFailActions?: ActionDef[];
  ...
  /** @deprecated 旧版「同屏气泡数上限」；运行时不再使用该字段（避免武断删掉某句对白），JSON 残留时忽略 */
