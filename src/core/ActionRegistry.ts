...
export function registerActionHandlers(executor: ActionExecutor, d: ActionRegistryDeps): void {
  executor.register('giveItem', (p) => d.inventoryManager.addItem(p.id as string, (p.count as number) ?? 1));
  executor.register('removeItem', (p) => d.inventoryManager.removeItem(p.id as string, (p.count as number) ?? 1));
  executor.register('giveCurrency', (p) => d.inventoryManager.addCoins(p.amount as number));
  executor.register('removeCurrency', (p) => d.inventoryManager.removeCoins(p.amount as number));
  executor.register('giveRule', (p) => d.rulesManager.giveRule(p.id as string));
  executor.register('giveFragment', (p) => d.rulesManager.giveFragment(p.id as string));
  executor.register('updateQuest', (p) => d.questManager.acceptQuest(p.id as string));
  ...
  executor.register('playBgm', (p) => d.audioManager.playBgm(p.id as string, (p.fadeMs as number) ?? 1000));
  executor.register('stopBgm', (p) => d.audioManager.stopBgm((p.fadeMs as number) ?? 1000));
  executor.register('playSfx', (p) => d.audioManager.playSfx(p.id as string));
  ...
  executor.register('inventoryDiscard', (p) => d.inventoryManager.discardItem(p.itemId as string));
}
