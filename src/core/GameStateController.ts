...
export class GameStateController {
  ...
  private handleEscape(): void {
    ...
    // 对话/遭遇里也能暂停（商业基本约定）；过场/小游戏各有自己的 Esc 通道，不在此列。
    const canPauseHere = this._currentState === GameState.Exploring
      || this._currentState === GameState.Dialogue
      || this._currentState === GameState.Encounter;
    if (canPauseHere && this.escapeFallback) {
      this.escapeFallback();
    }
  }
