...
function persistFavoritesToFile(keys: string[], log: (message: string) => void): void {
  if (!import.meta.env.DEV) {
    log('Flag 收藏仅能在 npm run dev 时写入 resources/editor_projects/editor_data/debug_flag_favorites.json');
    return;
  }
...
export interface DebugFlagSectionHandle {
  root: HTMLElement;
  destroy(): void;
  /** 从 resources/editor_projects/editor_data/debug_flag_favorites.json 重新加载（仅开发服有数据） */
...
export function createDebugFlagSection(
  ...
  favTitle.textContent = import.meta.env.DEV
    ? '收藏（resources/editor_projects/editor_data/debug_flag_favorites.json）'
    : '收藏（仅开发服持久化到仓库文件）';
  ...
  const refreshFavorites = (): void => {
    ...
    if (keys.length === 0) {
      ...
      empty.textContent = import.meta.env.DEV
        ? '暂无。在下方选好键后点「加入收藏」（写入 resources/editor_projects/editor_data/debug_flag_favorites.json）。'
