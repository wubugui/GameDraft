...
func _build() -> void:
	...
	container.name = "RuntimeLoreBookUI"
	...
	var title := Label.new()
	title.text = strings.get_text("loreBook", "title")
	...
	back_button.text = strings.get_text("loreBook", "back")
	...
	var lore_entries: Array = archive_data.get_unlocked_lore()
	...
	if lore_entries.is_empty():
		var empty := Label.new()
		empty.text = strings.get_text("loreBook", "empty")
		empty.position = Vector2(PADDING, cursor_y - 50.0)
		...
		for lore_value: Variant in lore_entries:
			if not lore_value is Dictionary:
				continue
			var entry: Dictionary = lore_value
			var entry_id := str(entry.get("id", ""))
			var is_new: bool = not archive_data.is_read("lore_%s" % entry_id)
			var label := Button.new()
			label.text = ("* " if is_new else "") + "[%s] %s" % [_category_label(str(entry.get("category", ""))), archive_data.resolve_line(entry.get("title", ""))]
			label.flat = true
			label.alignment = HORIZONTAL_ALIGNMENT_LEFT
			label.position = Vector2(PADDING, cursor_y - 50.0)
			...
			var select_lore := func(selected: Dictionary, selected_id: String, content_x: float, content_y: float) -> void:
				archive_data.trigger_first_view_if_needed("lore_%s" % selected_id, selected.get("firstViewActions"))
				archive_data.mark_read("lore_%s" % selected_id)
				_show_content(
					archive_data.resolve_line(selected.get("content", "")),
					archive_data.resolve_line(selected.get("source", "")),
					content_x,
					content_y,
				)
			label.pressed.connect(select_lore.bind(entry, entry_id, panel_x + CONTENT_X_OFFSET, panel_y + 50.0))
...
func _show_content(content_text: String, source: String, x: float, y: float) -> void:
	...
	var full_text := content_text + "\n\n%s %s" % [strings.get_text("loreBook", "source"), source]
...
func _category_label(category: String) -> String:
	return archive_data.get_lore_category_name(category)
