...
func show(text: String) -> void:
	...
	var screen_width := renderer.screen_width; var screen_height := renderer.screen_height; var width := minf(screen_width - 40.0, 600.0); var display_text := str(_resolve_display.call(text)) if not _resolve_display.is_null() and _resolve_display.is_valid() else text; var font := ThemeDB.fallback_font; var measured := font.get_multiline_string_size(display_text, HORIZONTAL_ALIGNMENT_LEFT, width - 40.0, 16); var height := minf(maxf(100.0, measured.y + 60.0), screen_height - 80.0); var position := Vector2((screen_width - width) / 2.0, screen_height - height - 30.0)
