...
func _ready() -> void:
	...
	var polygon: Array = RuntimeHotspot.collision_polygon_to_world(base); assert(polygon.size() == 3 and is_equal_approx(polygon[0].x, 925.0) and is_equal_approx(polygon[0].y, 1334.9) and is_equal_approx(polygon[2].x, 935.0) and is_equal_approx(polygon[2].y, 1354.9))
	var world_poly := base.duplicate(true); world_poly.collisionPolygonLocal = false; assert(RuntimeHotspot.collision_polygon_to_world(world_poly)[0] == {"x": -10.0, "y": -20.0})
	...
	var filter_probe := RefCounted.new(); hotspot.attach_depth_occlusion_filter(filter_probe); assert(hotspot.get_depth_occlusion_filter() == filter_probe and hotspot.detach_depth_occlusion_filter() == filter_probe and hotspot.get_depth_occlusion_filter() == null)
	...
	hotspot.set_position(50, 60); assert(hotspot.container.position == Vector2(50, 60) and RuntimeHotspot.collision_polygon_to_world(hotspot.def)[0] == {"x": 40.0, "y": 40.0})
	...
	assert(total == 115 and types == {"encounter": 2, "inspect": 73, "npc": 1, "pickup": 2, "transition": 37} and display == 21 and polygons == 12 and conditions == 46 and auto == 2 and cutscene == 8)
