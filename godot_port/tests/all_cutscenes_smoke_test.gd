...
func _ready() -> void:
	...
	bootstrap._debug_set_fixed_tick_mode(true)
	bootstrap.cutscene_renderer.set_time_scale(0.0); bootstrap.emote_bubble_manager.set_time_scale(0.0)
	var ids: Array = bootstrap.cutscene_manager.get_cutscene_ids(); assert(ids.size() == 20)
	for argument: String in OS.get_cmdline_user_args():
		if argument.begins_with("--cutscene-id="): ids = [argument.trim_prefix("--cutscene-id=")]
	for id: String in ids:
		...
		bootstrap.action_executor.execute_await({"type": "startCutscene", "params": {"id": id}})
		if id == "洋人第一次出场":
			var staging_guard := 0
			...
			assert(SceneQueries.hotspot(bootstrap.scene_manager, "new_hotspot_6") != null and bootstrap.scene_manager.get_active_cutscene_binding_id() == id)
		var guard := 0
		while bootstrap.cutscene_manager.is_playing() and guard < 900:
			guard += 1; bootstrap.cutscene_manager.wait_click_not_before = 0; bootstrap.cutscene_manager.dialogue_advance_not_before = 0; InputManagerProbe.pointer_down(bootstrap.input_manager); bootstrap.tick(0.1); await get_tree().process_frame
		assert(guard < 900 and not bootstrap.cutscene_manager.is_playing() and bootstrap.action_executor.get_policy_depth() == 0, "cutscene=%s guard=%s playing=%s policyDepth=%s step=%s" % [id, guard, bootstrap.cutscene_manager.is_playing(), bootstrap.action_executor.get_policy_depth(), bootstrap.cutscene_manager.get_playback_hud_snapshot()])
		...
		if id == "洋人第一次出场":
			...
			assert(dock_memory == null, "cutscene staging must not materialize committed scene memory")
