...
func _run() -> void:
	...
	var evaluator: RuntimeConditionEvaluator = bootstrap.condition_evaluator

	assert(evaluator.evaluate({"flag": "box_opened"}, context))
	assert(evaluator.evaluate({"quest": "opening_01", "questStatus": "Active"}, context))
	assert(evaluator.evaluate({"scenario": "integration", "phase": "phase", "status": "done", "outcome": "ok"}, context))
	assert(evaluator.evaluate({"scenarioLine": "码头水鬼", "lineStatus": "active"}, context))
	assert(evaluator.evaluate({"narrative": "flow_dock_water_monkey", "state": initial}, context))
	assert(evaluator.evaluate({"narrative": "flow_dock_water_monkey", "state": initial, "reached": true}, context))
	assert(evaluator.evaluate({"plane": "背尸"}, context))
	assert(evaluator.evaluate({"all": [{"flag": "box_opened"}, {"quest": "opening_01", "questStatus": "Active"}]}, context))
	assert(evaluator.evaluate({"any": [{"flag": "got_iron_box"}, {"plane": "背尸"}]}, context))
	assert(evaluator.evaluate({"not": {"flag": "got_iron_box"}}, context))
	assert(not evaluator.evaluate({"unknown": true}, context))
	...
	print("Live 9-node condition provider test: PASS")
	get_tree().quit(0)
