...
func _ready() -> void:
	...
	assert(bootstrap.cutscene_manager.has_cutscene("说书-李天狗大战旱魃") and bootstrap.audio_manager.has_audio("sfx", "sfx_jingju_luogu"))
	...
	bootstrap.cutscene_manager.set_time_scale(0.0)
	...
	while bootstrap.cutscene_manager.is_playing() and guard < 600:
		guard += 1
		bootstrap.cutscene_manager.debug_advance()
		await get_tree().process_frame
	...
	assert(bootstrap.action_executor.policy_depth() == 0 and bootstrap.cutscene_renderer.get_active_image_handles().is_empty())
	# 这些 actor 原语的生产契约是过场内执行；TS 也只在 Cutscene tick 临时 actor。
	bootstrap.state_controller.set_state(RuntimeGameStateController.CUTSCENE)
	...
	var actor: Variant = bootstrap.cutscene_manager.get_temp_actor("_probe")
	...
	assert(bootstrap.cutscene_manager.get_temp_actor("_probe") == null)
	...
	bootstrap.state_controller.set_state(RuntimeGameStateController.EXPLORING)
