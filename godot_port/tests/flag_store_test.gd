...
func _init() -> void:
	...
	assert(FlagStoreProbe.registry_counts(flags) == {"static": 77, "patterns": 18})
	...
	assert(flags.get_value(" ") == null)
	flags.set_value("player_health", 5.0)
	flags.set_value("player_health", 5.0)
	assert(changes.size() == 1)
	flags.set_value("player_health", true)
	assert(changes.size() == 2)
	flags.add_numeric_flag("player_health", 2.0)
	assert(flags.get_value("player_health") == 2.0)
	flags.add_numeric_flag("current_smell", 1.0)
	assert(flags.get_value("current_smell") == null)
	flags.append_string_flag("current_smell", "香")
	flags.append_string_flag("current_smell", "火")
	assert(flags.get_value("current_smell") == "香火")
	flags.append_string_flag("player_health", "x")
	assert(flags.get_value("player_health") == 2.0)
	...
	assert(flags.eval_pure_flag_conjunction([{"flag": "one", "op": null, "value": 1.0}]))
	assert(flags.eval_pure_flag_conjunction([{"flag": "numeric_string", "op": ">", "value": 2.0}]))
	flags.set_value("binary_string", "0b10")
	flags.set_value("octal_string", "0o10")
	assert(flags.eval_pure_flag_conjunction([{"flag": "binary_string", "op": ">", "value": 1.0}]))
	assert(flags.eval_pure_flag_conjunction([{"flag": "octal_string", "op": "==", "value": "0o10"}]))
	assert(flags.eval_pure_flag_conjunction([{"flag": "octal_string", "op": ">=", "value": 8.0}]))
