...
func _ready() -> void:
	...
	assert(toast.size == Vector2(240, 30), "toast dimensions must match Pixi"); assert(toast.position == Vector2((bootstrap.renderer.screen_width - 240) / 2.0, 50), "toast anchor must match Pixi")
	...
	InputManagerProbe.key_down(bootstrap.input_manager, "KeyI"); InputManagerProbe.key_up(bootstrap.input_manager, "KeyI"); assert(bootstrap.inventory_ui.is_open() and bootstrap.state_controller.current_state == RuntimeDataTypes.UI_OVERLAY and bootstrap.inventory_ui.content.text.contains("桃木"))
	...
	InputManagerProbe.key_down(bootstrap.input_manager, "Escape"); InputManagerProbe.key_up(bootstrap.input_manager, "Escape"); assert(not bootstrap.inventory_ui.is_open() and bootstrap.state_controller.current_state == RuntimeDataTypes.EXPLORING)
	await bootstrap.action_executor.execute_await({"type": "updateQuest", "params": {"id": "opening_01"}})
	InputManagerProbe.key_down(bootstrap.input_manager, "Tab"); InputManagerProbe.key_up(bootstrap.input_manager, "Tab"); assert(bootstrap.quest_panel_ui.is_open() and bootstrap.quest_panel_ui.content.text.length() > 10); InputManagerProbe.key_down(bootstrap.input_manager, "Escape"); InputManagerProbe.key_up(bootstrap.input_manager, "Escape")
	await bootstrap.action_executor.execute_await({"type": "giveRule", "params": {"id": "rule_no_go_night"}})
	InputManagerProbe.key_down(bootstrap.input_manager, "KeyR"); InputManagerProbe.key_up(bootstrap.input_manager, "KeyR"); assert(bootstrap.rules_panel_ui.is_open() and bootstrap.rules_panel_ui.content.text.length() > 10); InputManagerProbe.key_down(bootstrap.input_manager, "Escape"); InputManagerProbe.key_up(bootstrap.input_manager, "Escape")
