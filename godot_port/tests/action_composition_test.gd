...
func _ready() -> void:
	...
	preload("res://tests/support/action_registry_fixture.gd").register(executor, {
		...
		"resolveDisplayText": func(text: String) -> String: return text.replace("{{name}}", "阿明"),
		"chooseAction": Callable(self, "_choose"),
		"randomValue": func() -> float: return random_sample,
	})
	...
	assert(log == [{"id": "a", "zone": "zone_a"}, {"id": "b", "zone": "zone_a"}] and state.current_state == RuntimeDataTypes.EXPLORING)
	...
	assert(choice_calls == [{"prompt": "你好 阿明", "options": [{"text": "第一 阿明"}, {"text": "第二"}], "allowCancel": true, "state": RuntimeDataTypes.UI_OVERLAY}] and log[-1] == {"id": "second", "zone": "choice_zone"} and state.current_state == RuntimeDataTypes.EXPLORING)
