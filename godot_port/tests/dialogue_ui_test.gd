...
func _ready() -> void:
	var repository := ProjectSettings.globalize_path("res://").trim_suffix("/").get_base_dir(); var assets := RuntimeAssetManager.new({}, RuntimeResourceLocator.new(RuntimeResourceLocator.DEVELOPMENT, repository)); var events := RuntimeEventBus.new(); var strings := RuntimeStringsProvider.new(); strings.load(assets); var input := RuntimeInputManager.new(); add_child(input); var renderer := RuntimeRenderer.new(); add_child(renderer); renderer.set_asset_manager(assets); renderer.init(); renderer.set_viewport_size(800, 600)
	...
	events.emit("dialogue:end", {"source": "graph", "willContinue": false}); assert(not ui.is_open() and InputManagerProbe.subscriber_count(input) == 0)
	ui.destroy(); assets.dispose(); input.destroy(); remove_child(input); input.free(); renderer.destroy(); remove_child(renderer); renderer.free(); events.clear()
