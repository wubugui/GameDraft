...
func _ready() -> void:
	...
	var registry := RuntimeNpc.build_character_registry(registry_raw); assert(registry.size() == 4)
	var inherited := RuntimeNpc.apply_character_defaults({"id": "clara_here", "characterId": "clara", "name": "", "x": 10, "y": 20, "interactionRange": 80}, registry)
	...
	var local_wins := RuntimeNpc.apply_character_defaults({"id": "local", "characterId": "clara", "name": "本地名", "animFile": "/resources/runtime/animation/埃德加_anim/anim.json", "x": 1, "y": 2, "interactionRange": 3}, registry)
	...
	assert(RuntimeNpc.portrait_slug_from_anim_file("/resources/runtime/animation/克拉拉_anim/anim.json") == "克拉拉_anim")
	...
	if not sprite_loaded: print("NPC sprite load diagnostic: ", inherited.animFile, " / ", assets.last_error, " / ", assets.debug_snapshot())
	...
	assert(scene_files.size() == 27 and npc_count == 63 and character_refs == 9 and animated == 53 and patrols == 11)
