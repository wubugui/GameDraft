...
func _ready() -> void:
	var renderer := RuntimeRenderer.new(); add_child(renderer); renderer.init_renderer()
	...
	assert(renderer.get_screen_width() == 640 and renderer.get_screen_height() == 360 and resize_count == 1)
	renderer.set_window_size(1024, 768); assert(renderer.get_window_size_request() == {"width": 1024, "height": 768} and resize_count == 2)
	unsubscribe.call(); renderer.set_viewport_size(800, 600); assert(resize_count == 2)
	...
	renderer.set_world_filters([{"matrix": matrix, "alpha": 0.75}]); assert(renderer.get_world_filters().size() == 1 and renderer.world_filter_pipeline.has_filters() and renderer.world_container.material is ShaderMaterial)
	...
	renderer.clear_world_filter(); assert(renderer.get_world_filters().is_empty() and renderer.world_container.material == null)
	renderer.destroy_renderer(); assert(not renderer.is_initialized() and renderer.get_child_count() == 0 and renderer.get_screen_width() == 800 and renderer.get_screen_height() == 600)
