...
func _ready() -> void:
	...
	assert(bootstrap.action_executor.has_handler("playScriptedDialogue") and bootstrap.dialogue_manager.is_active() and bootstrap.dialogue_ui.is_open() and bootstrap.state_controller.current_state == RuntimeDataTypes.DIALOGUE)
	var guard := 0
	...
	assert(lines[1].speaker == "关二狗" and lines[1].speakerEntity == {"kind": "player"} and lines[1].portrait.slug == "player_anim" and lines[1].dim == true)
	...
	assert(not bootstrap.dialogue_manager.is_active() and not bootstrap.dialogue_ui.is_open() and bootstrap.state_controller.current_state == RuntimeDataTypes.EXPLORING)
	var graph: RuntimeGraphDialogueManager = bootstrap.graph_dialogue_manager; graph.graph = {"entry": "script", "nodes": {"script": {"type": "runActions", "actions": [{"type": "playScriptedDialogue", "params": {"lines": [{"speaker": "{{npc}}", "text": "嵌套脚本。"}]}}], "next": "outer"}, "outer": {"type": "line", "speaker": {"kind": "npc"}, "text": "外层继续。", "next": "end"}, "end": {"type": "end"}}}; graph.graph_source_id = "nested_script"; graph.current_node_id = "script"; graph.npc_name = "说书人张叨叨"; graph.npc_id = "storyteller_zhang"; graph.owner_type = "npc"; graph.owner_id = "storyteller_zhang"; graph.active = true; bootstrap.state_controller.set_state(RuntimeDataTypes.DIALOGUE); graph._drain_until_blocking(); await get_tree().process_frame
	assert(graph.is_active() and bootstrap.dialogue_manager.is_active() and bootstrap.state_controller.current_state == RuntimeDataTypes.DIALOGUE); bootstrap.dialogue_ui.update(100.0); bootstrap.dialogue_ui.debug_advance(); await get_tree().process_frame; assert(not bootstrap.dialogue_manager.is_active() and graph.is_active() and lines[-1].text == "外层继续。" and bootstrap.state_controller.current_state == RuntimeDataTypes.DIALOGUE); bootstrap.dialogue_ui.update(100.0); bootstrap.dialogue_ui.debug_advance(); await get_tree().process_frame; assert(not graph.is_active() and bootstrap.state_controller.current_state == RuntimeDataTypes.EXPLORING)
