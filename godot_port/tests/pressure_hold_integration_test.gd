...
func _ready() -> void:
	...
	assert(bootstrap.pressure_hold_manager.get_def_count() == 8)
	assert(bootstrap.pressure_hold_manager.register_def_for_test({
	...
	assert(saw_overlay and bootstrap.state_controller.current_state == RuntimeGameStateController.EXPLORING)
	...
	print("PressureHold Action/UIOverlay/JSON/state integration test: PASS")
	get_tree().quit(0)


func _capture_overlay_and_drive() -> void:
	saw_overlay = bootstrap_under_test.state_controller.current_state == RuntimeGameStateController.UI_OVERLAY and bootstrap_under_test.pressure_hold_ui.get_root() != null
...
func _drive_segment_edges() -> void:
	...
	for _index: int in 120:
		...
		if not bootstrap_under_test.pressure_hold_manager.is_running(): return
		await get_tree().process_frame
