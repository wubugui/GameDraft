...
func _ready() -> void:
	var renderer := RuntimeRenderer.new(); add_child(renderer); renderer.init(); renderer.set_viewport_size(800, 600); var strings := RuntimeStringsProvider.new(); var input := RuntimeInputManager.new(); add_child(input); var box := RuntimeInspectBox.new(renderer, strings, input); box.set_resolve_display(func(text: String) -> String: return text.replace("[name]", "阿明"))
	box.show("你好 [name]"); assert(box.is_open()); var body: Label = box.root.get_child(1); assert(body.text == "你好 阿明")
	await get_tree().create_timer(0.12).timeout; InputManagerProbe.key_down(input, "KeyE"); await get_tree().process_frame; assert(not box.is_open() and InputManagerProbe.subscriber_count(input) == 0); InputManagerProbe.key_up(input, "KeyE")
	...
	box.destroy(); input.destroy(); remove_child(input); input.free(); renderer.destroy(); remove_child(renderer); renderer.free()
