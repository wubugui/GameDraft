...
func _init() -> void:
	...
	provider.load(assets)
	var counts: Dictionary = preload("res://tests/support/strings_provider_probe.gd").counts(provider)
	assert(counts.categories == 31 and counts.leaves == 185)
