...
func _ready() -> void:
	var events := RuntimeEventBus.new(); var flags := RuntimeFlagStore.new(events); var executor := RuntimeActionExecutor.new(events, flags); var planes := RuntimePlaneReconciler.new(events); planes.register_defs([{"id": "normal", "membership": "shared"}, {"id": "dream", "membership": "exclusive"}]); preload("res://tests/support/action_registry_fixture.gd").register(executor, {"planeReconciler": planes})
