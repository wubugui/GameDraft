...
func _ready() -> void:
	...
	var manager := RuntimeSceneManager.new(assets, events, renderer, player, camera); add_child(manager); manager.init({}); events.on("scene:transition", Callable(self, "_capture_transition")); events.on("scene:beforeUnload", Callable(self, "_capture_before_unload")); events.on("scene:enter", Callable(self, "_capture_scene_enter")); events.on("scene:ready", Callable(self, "_capture_scene_ready")); events.on("scene:entitiesRebuilt", Callable(self, "_capture_entities_rebuilt"))
	assert(manager.load_scene("teahouse")); assert(manager.set_entity_runtime_field("teahouse", "npc", "storyteller_zhang", "x", 321).ok); assert(manager.set_entity_runtime_field("teahouse", "npc", "storyteller_zhang", "y", 222).ok); assert(manager.set_entity_runtime_field("teahouse", "npc", "storyteller_zhang", "enabled", false).ok); assert(not manager.set_entity_runtime_field("teahouse", "npc", "storyteller_zhang", "bogus", 1).ok)
	var storyteller: RuntimeNpc = manager.get_npc_by_id("storyteller_zhang"); assert(storyteller.get_x() == 321 and storyteller.get_y() == 222 and not storyteller.container.visible)
	assert(manager.set_entity_session_enabled("hotspot", "exit_to_street", false)); assert(not manager.get_hotspot_by_id("exit_to_street").container.visible)
	...
	assert(manager.switch_scene("test_room_b")); events.emit("hotspot:inspected", {"hotspotId": "strange_mark"}); events.emit("hotspot:pickup:done", {"hotspotId": "herb_bundle"}); assert(manager.get_hotspot_by_id("herb_bundle").get_picked_up())
	assert(manager.switch_scene("teahouse", "from_street")); assert(player.get_x() == 151.9 and player.get_y() == 171.4); storyteller = manager.get_npc_by_id("storyteller_zhang"); assert(storyteller.get_x() == 321 and storyteller.get_y() == 222 and not storyteller.container.visible and not manager.get_hotspot_by_id("exit_to_street").container.visible)
	...
	manager.deserialize(wire); assert(manager.load_scene("test_room_b")); assert(manager.get_hotspot_by_id("herb_bundle") == null and manager.get_hotspot_by_id("taomu_pickup") != null)
	assert(manager.load_scene("teahouse")); assert(not manager.get_npc_by_id("storyteller_zhang").container.visible and manager.get_hotspot_by_id("exit_to_street").container.visible)
	...
	assert(manager.load_scene("teahouse")); assert(manager.get_npc_by_id("blind_li").get_x() == 444 and manager.is_npc_patrol_persistently_disabled("blind_li") and manager.get_hotspot_by_id("teahouse_notice").has_depth_display_image())
	...
	assert(manager.load_scene("码头白天", "", {"x": 901.0, "y": 777.0}, null, false)); lifecycle_events.clear(); rebuilt_events.clear()
	var background_before := manager.scene_background; var unbound_before: RuntimeHotspot = manager.get_hotspot_by_id("hotspot_码头告示板"); var outer_before: RuntimeHotspot = manager.get_hotspot_by_id("new_hotspot_人群")
	assert(manager.get_npc_by_id("new_npc_3") == null); assert(manager.get_hotspot_by_id("new_hotspot_6") == null)
	assert(manager.begin_cutscene_staging("洋人第一次出场", "码头白天")); assert(manager.enter_cutscene_instances_for_current("洋人第一次出场"))
	assert(lifecycle_events.is_empty() and manager.scene_background == background_before and manager.get_hotspot_by_id("hotspot_码头告示板") == unbound_before)
	assert(manager.get_hotspot_by_id("new_hotspot_人群") != outer_before and manager.get_npc_by_id("new_npc_3") != null and manager.get_hotspot_by_id("new_hotspot_6") != null)
