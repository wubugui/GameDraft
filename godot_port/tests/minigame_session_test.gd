...
class FakeManager:
	...
	func get_index_url() -> String:
		return "/assets/data/paper_craft/index.json"

	func get_data_subdir() -> String:
		return "paper_craft"

	func get_scope_prefix() -> String:
		return "minigame:test"
...
func _ready() -> void:
	...
	renderer = RuntimeRenderer.new(); add_child(renderer); renderer.set_asset_manager(assets); renderer.init_renderer()
	manager = FakeManager.new(); add_child(manager); manager.init({"assetManager": assets}); manager.bind_session_runtime({"renderer": renderer, "inputManager": input, "stateController": state})
	assert(manager.load_index() and manager.get_instance_list() == [{"id": "wujin_paper_servant_daywork", "label": "雾津纸扎铺：糊纸人日工"}])
	...
	assert(saw_scene_attached and not manager.active and state.current_state == RuntimeGameStateController.EXPLORING and not input.is_key_down("Space"))
	...
	assert(escaped == null and duplicate_returned_null and state.current_state == RuntimeGameStateController.EXPLORING and not manager.active)
	assert(manager.load_calls == 2 and manager.teardown_hooks == 2)
	assert(await manager.run_until_done("unknown") == null and state.current_state == RuntimeGameStateController.EXPLORING)
	...
	state.set_state(RuntimeGameStateController.MINIGAME)
	get_tree().process_frame.connect(Callable(self, "_capture_gate_lock").bind(gate), CONNECT_ONE_SHOT)
	await gate.run([{"type": "probe"}])
	assert(saw_gate_locked and not gate.is_locked() and gate_lock_changes == [true, false] and gate_restore_count == 1 and state.current_state == RuntimeGameStateController.MINIGAME)
	...
	assert(not manager.active and state.current_state == RuntimeGameStateController.EXPLORING and manager.scene == null and assets.get_stats().json.pinned == 0)
	remove_child(manager); manager.free(); state.destroy(); input.destroy(); renderer.destroy_renderer(); remove_child(input); input.free(); remove_child(renderer); renderer.free(); assets.dispose()
	print("MinigameSession duplicate/Esc/scope/state/destroy/action-gate contract test: PASS")
	get_tree().quit(0)
...
func _complete_session() -> void:
	...
	manager.publish_result({"status": "success"})
...
func _duplicate_then_escape() -> void:
	...
	input.debug_key_down("Escape")
	input.debug_key_up("Escape")
