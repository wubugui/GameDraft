...
func _ready() -> void:
	...
	var npc: RuntimeNpc = bootstrap.scene_manager.get_npc_by_id("storyteller_zhang"); assert(npc != null); var original_zoom: float = bootstrap.camera.get_zoom()
	var routed: bool = await bootstrap.interaction_coordinator.debug_interact_npc_by_id("storyteller_zhang"); assert(routed); assert(bootstrap.graph_dialogue_manager.is_active() and bootstrap.dialogue_ui.is_open() and bootstrap.state_controller.current_state == RuntimeGameStateController.DIALOGUE and npc.is_patrol_paused_for_dialogue())
	...
	assert(bootstrap.state_controller.current_state == RuntimeGameStateController.EXPLORING and not npc.is_patrol_paused_for_dialogue() and bootstrap.camera.get_zoom() == original_zoom and bootstrap.emote_bubble_manager.active_bubbles.is_empty())
