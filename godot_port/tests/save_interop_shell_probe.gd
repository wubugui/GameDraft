...
func _ready() -> void:
	...
	OS.set_environment(RuntimeLocalStorage.ROOT_ENV, storage)
	var bootstrap: Node = BootstrapScript.new(); bootstrap.set_meta("suppressSceneOnEnter", true); add_child(bootstrap)
	var boot_guard := 0
	while bootstrap.runtime_command_bridge == null and boot_guard < 30:
		boot_guard += 1
		await get_tree().process_frame
	assert(boot_guard < 30)
	# Readiness and Game.tick are separate contracts: even a synchronous bootstrap
	# still needs real ticks before playTimeMs can be serialized.
	await get_tree().process_frame
	await get_tree().process_frame
	var manager := RuntimeSaveManager.new(Callable(bootstrap, "collect_save_data"), Callable(bootstrap, "distribute_save_data"), Callable(bootstrap, "reload_saved_scene"), bootstrap.strings_provider, "teahouse")
	...
	if phase == "produce":
		...
		assert(local_snapshot.get("dialogueLog") is Dictionary and local_snapshot.dialogueLog.entries.size() == 1 and float(local_snapshot.game.playTimeMs) > 0.0 and int(local_snapshot.game.randomState) == bootstrap.runtime_random.get_state())
		var expected_next_random: float = bootstrap.runtime_random.next()
		assert(bootstrap.distribute_save_data(local_snapshot) and restore_events[0] == 1 and bootstrap.dialogue_log_ui.entries.size() == 1)
		assert(bootstrap.runtime_random.next() == expected_next_random)
		...
		assert(await bootstrap.reload_saved_scene("teahouse"))
		await get_tree().process_frame
		...
		assert(manager.save(0)); assert(_write_text(output, manager.export_slot_payload(0)))
		print("Godot save interop producer: PASS")
	else:
		var input := str(args.get("input", "")); assert(not input.is_empty())
		assert(manager.import_slot_payload(0, _read_text(input)) and await manager.load(0))
		...
		assert(manager.save(1)); assert(_write_text(output, manager.export_slot_payload(1)))
		print("Godot save interop consumer: PASS")
	bootstrap.audio_manager.stop_all_playback(); bootstrap.asset_manager.clear_cache(); remove_child(bootstrap); bootstrap.free(); await get_tree().create_timer(0.1).timeout; get_tree().quit(0)
...
func _write_text(path: String, raw: Variant) -> bool:
