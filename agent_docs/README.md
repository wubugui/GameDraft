...
2. **按改动文件找**:`sh scripts/py.sh agent_docs/_meta/audit.py --paths <将改动的文件...>`
   → 输出这些路径登记的必读机制卡。
...
- **统一入口 = 治理台 CLI**:遇到任何治理类业务先跑
  `sh scripts/py.sh agent_docs/_meta/cli.py list`(或 `route "业务一句话"`),现场发现流程,
...
- 机械体检+索引重生成:`sh scripts/py.sh agent_docs/_meta/cli.py audit`(门禁模式加 `--check`)。
