...
需求属"做内容"(任务/对话/遭遇/规矩/物品/演出/场景交互/档案/地图/小游戏/文案…)。
...
3. **L2 升级(条件触发)**——目的:缺一个能力原语时最小新增。完成判据:登记面三件套
   齐 + TS 编译过 + 明确告知用户改了哪个原语/哪些文件。
4. **收尾校验**——完成判据:双校验门干净,且 warning 逐条看过。
5. **汇报**——完成判据:列出改动文件、L2 改动、校验结果、全部 L3 跳过任务及建议。

## 判断点

- **L1/L2/L3 判定**(核心判断):纯靠已注册原语 + 条件叶子布尔组合拼得出 = **L1**;
  只缺一个原语(新 command / 缺参数 / 新 present 类型 / 新条件叶子 / 新图节点)= **L2**;
  需要运行时算术、跨变量取值、集合寻址或新子系统 = **L3**——不硬做,跳过并汇报。
- **先证明不是 L1 没拼对**,再动 L2;L2 只加"打通需求所需的最小原语"。
- **盲区即升级信号**:需求落在"运行时支持但编辑器 GUI 改不到"的字段上,按 L2 补编辑器
  支持或上报,不闷头写人类维护不了的 JSON。
- **演出归属**:有时序/相机/淡入淡出/并行 → cutscene;单发反馈可作普通 command。
...
- 在嵌套结构里塞编辑器不认识的字段——落在重建区就被人类开一次面板抹掉。
- 把"能 json.loads"当校验通过——两道校验门都不跑等于没验。
- 把一次内容需求扩成大重构。

## 向下指针

- **做叙事/事件内容的 full 能力(涉及事件必载)**:[narrative-flow-authoring](narrative-flow-authoring.md)
