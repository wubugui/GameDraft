...
糖画转盘 `atmosphereGroups[]` 各阶段脚本的编辑器:每条指令一行、条件类 op 在行下缩进挂 then/else 子列表的递归控件,挂在转盘编辑器的阶段标签页里。

## 权威源(读代码从哪进)

- 编辑器:`tools/editor/editors/atmosphere_script_editor.py`
- 消费方:`src/systems/sugarWheel/sugarWheelAtmosphere.ts`(**独立轻量运行时,不是 ActionExecutor**)

## 硬契约

1. `to_list()` 产出的 step 结构必须与该运行时逐字段一致,数据格式不变。
...
3. `say` 的「随机抽池」与「固定台词」靠标记项区分,读回靠文本比对而非 currentIndex;改名文案池必须递归传播到引用它的步骤。

## 已知坑

- 氛围 op 与通用 action 语义不同(近扇区条件、池抽取是 wheel 专属)——别把它们登进 `ACTION_TYPES` 或让 ConditionEditor 管。
