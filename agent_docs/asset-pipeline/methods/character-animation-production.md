...
1. **源确认**——锁定输入源。完成判据:源 = 游戏当前实际源,以 shipped `atlas.meta.json` 的
   packMode/source 逐角色查证,不凭目录名猜。
2. **生成侧出视频**——拿到各 state 的原地干净视频。完成判据:原地(跑步机式)、道具全程握持、
   身份画风一致。
3. **程序产出**——跑确定性产线。完成判据:硬闸门全过(程序退出码为准)。
4. **QA 裁决**——agent 对程序报出的每条软 flag 给结构化结论。完成判据:每条 flag 都有
   accept/reject。
...
- **reject 后升级阶梯**:重摇 K 次(同参数)→ quality 升 high → 换生成模式(动作迁移↔单图)
  → 叫人。
