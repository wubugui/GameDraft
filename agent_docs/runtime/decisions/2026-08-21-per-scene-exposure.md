...
## ⚠ 2026-08-30 模型翻转后的订正(拍板本身不变)

`display` 逐场景独立调这条**继续成立**,而且现在它同时作用于背景与角色
(`applyDisplay`,少了这条就是"背景很亮、角色漆黑")。上面三条技术事实要改两条:

- **②已失效**:`radianceScale` 那条接线在统一角色路径里,该路径已被
  `Game.UNIFIED_CHAR_PATH_ENABLED = false` 关死 ⇒ 这个数**当前无消费者**。
- **③已改写**:背景不再走 `原画 × S_new/S_day`,而是 `原画 + 反解albedo × 实体灯`
  (见 [[scene-lighting]])。原画的绝对亮度**直接就是画面亮度**,所以"背景亮度随原画
  浮动、只靠手挑 ev 补偿"这个处境**没变**,反而更直白了 —— 本决定接受的代价原样成立。

下面「边界」一节里点名 `radianceScale` 的那条,在该路径复活之前是空转的。
