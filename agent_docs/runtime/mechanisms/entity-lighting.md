...
- **接触阴影 = 胶囊 AO,在 M-world wu 里算,尺度绝不按帧宽取**(2026-09-24,制作人要"带方向性的 AO,像 3D 胶囊体 AO"):
  ...
  从**最低的不透明行**往上 `CONTACT_BAND` 取范围、按帧缓存);**半宽与中心按角色定死**:只在站立片段
  (`SpriteEntity.getBodyReferenceFrames`:逻辑 `idle` 经 stateMap、按该片段自己的 footOffset 裁底;没有 idle 取图集
  第一个片段)上量、逐帧取中位数(`bodyFootprintOf`,按帧组缓存),走 / 跑 / 蹲沿用;当前帧只判「底部有没有东西
  挨地」(null 就不画)。按当前帧量时步幅让半径帧帧变(主角走 8~29、跑 4~37 世界 px,中心晃到 35),接触 AO
  跟着动画一跳一跳(制作人 2026-09-25 真机);站着的样子两种算法逐像素相同。高 = 帧高换算的世界高。地面片元用行走面深度场
  还原成 M-world(wu,铁律 0)后算两部分:
  ① 无方向 = **胶囊**(与②同一根:底端球心高 r、贴地只挨一点)对地面点的余弦加权遮蔽(`capsuleOmni`:
  按方位角切竖直半平面,每片里胶囊截面是凸的 2D 胶囊,被挡仰角一整段 [lo,hi],余弦加权 (sin²hi−sin²lo)/2,
  8 片中点求积,与 40 万射线蒙特卡洛差 < 0.006;编辑器预览 `light_env_visual._omni` 同式,对账测试钉物理值)。
  贴地那一点正好 1、往外平滑落下(x=r 0.354、2r 0.149),「明暗」就是脚边最暗处,不归一。
  ⚠ 2026-09-25 前按**平底实心圆柱** `(asin(r/x)/π)·h²/(h²+x²)` 算、贴身体表面归一到 1:柱下一整圈恒 1、
  柱边 asin 斜率无穷断崖,真机是脚后一块没有渐变的黑饼(制作人),且与②的胶囊几何对不上——别改回去。
  顶只到身高的「晕开」比例(作者参数 spread)——这一层画在已打好光的画面上分不出环境光与灯光,全高的 1/x 长尾
  ...
  **方向来源是作者选项 `dirSource`**,缺省 `lighting`(按光照)= **跟角色身上的光一致**(制作人 2026-09-24:
  ...
  29 套有脚离帧底 > 6% 帧高的帧);圆柱内部按 1、轴放在脚尖线上(脚前一块硬边黑盘子);**按当前帧量胶囊宽度**
  (跟步幅一跳一跳,见上)。按剪影自动拆两条小腿胶囊也试过(离线,主角图集):走 16 帧里 8 帧、跑 6 帧两腿在剪影里
  叠成一块拆不开,硬拟合出横着的胖胶囊;能拆开的帧左右腿身份逐帧对调。脚的接触要做只能走逐帧挂点 + 动画浏览页人工修,
  制作人 2026-09-25 暂缓。
