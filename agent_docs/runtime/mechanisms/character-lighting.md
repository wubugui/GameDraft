...
着色核心 `charShadeCore.glsl#shadeEntityLinear`(运行时角色/粒子三项倍率；
`shadeCharacterLinear` 留给旧参数实验室，两者共用同一份源码);
...
- **受光倍率逐场景 / 时段，角色与粒子独立**(2026-09-14)：
  `lighting.lightFactors.character / particles` 各含 `indirectFactor / directFactor / totalFactor / eChroma`，
  反射光为 `linear(color) × 总倍率 × (间接E × 间接倍率 + 直接E × 直接倍率)`，再走原有色度与显示变换。
  主编辑器「角色 / 粒子受光（场景保存值）」编辑正式作者参数；时段表单可覆盖整组。
  F2「光影→照明」编辑两组**运行时实时值**，经原有光照同步通道进入编辑器工作副本，
  `Save All` 才写正式场景文件；不把同步槽当成存档，不写进玩家存档。
  旧场景缺项等价取 `间接=giStrength、直接=1、总=2^beta/π`，保证接线不改既有画面；
  **显式配置优先**，已经人工标定的场景倍率不能被旧 beta 再乘一次。
  `eChroma` 是各组独立保存的光色度，0=只接受明暗、1=完整接受光色；当前仍作用于间接+直接的合成光。
  粒子的场景采样组与角色分开，角色曝光 / 色度 / 测试太阳不再串到粒子；
  粒子效果只额外乘自己的 `appearance.lightGain`，自发光份额不参与这些倍率。
  2026-09-14 已按原有效值补全 35 个场景 / 47 套外观的两组参数；以后重烘角色旧 beta/eChroma
  不得覆盖场景已保存的粒子参数。新旧载荷缺项兼容仍在，但不是现有场景的作者真值。
...
- **着色核心单一真相源**:只改 `charShadeCore.glsl`,禁止在任一 shader 内联重写。
  实验室仍负责烘焙/采样初值；场景受光倍率在 F2 调节并随场景保存，不能把它们写回效果定义。
...
- **载荷旁挂文件缺席必抛,不许把 404 正文当数据**(2026-09-06):`lighting.json` 在而它
  指定的 probe 图集 / `probes_valid.bin` / `ground_d.png` 取不到(HTTP 非 2xx 或回落成 HTML)时,
  `fetchPayloadBytes` 抛出带路径的错误,整份载荷作废并 `depthError` 出声(两档都进 console)。
  以前是裸 `r.arrayBuffer()`:发行包漏抽 `atlas_bin.bin` 时 Tauri 的 404 正文被当图集吃进去,
  偶数字节补零成全黑(角色纯黑剪影、背景照常),奇数字节 `Uint16Array` 抛 RangeError 整份作废
  (角色不打光 + 深度场一起丢)——两种互相矛盾的表现同根,28 个场景黑了一轮而零报错。
  运行时**读哪些文件**只在 `src/core/lightingPayloadFiles.ts` 定义(mode→图集表、四组文件名),
  `probeCfg` 与打包展开器都查它;进场景只拉载荷 `shading.mode` 那一张,**缺省是 mode 3 八面体**
  (2026-09-02 正式档;"游戏默认 L2=9列"是旧口径)。
...
- **`shading.beta` 是补偿不是修复**:它是为了补"角色比紧邻地面暗一大截"抬起来的,
  ...
  现状:probe 只存原画反弹 + 逃逸地板色、不含照亮原画的那份天光,原始底光普遍只有"原画÷albedo"的几个百分点到十几个百分点,
  全靠 β 拉回来;批量烘焙缺省 β=0 的场景角色就是黑剪影(46 份载荷里 09-14 前只有雾津街头 4.2 / 茶馆 3.0 调过)。
  **跑马梁 2026-09-14 按游戏内实测对齐雾津街头**:度量 M =(角色渲染亮度 ÷ 贴图亮度)÷(脚边朝上地面渲染亮度 ÷ albedo),
  同一张玩家贴图在多处取中位;雾津街头白天 0.472、夜(关灯)0.464 → 跑马梁白天 β=3.0(实测 0.468)、夜 β=3.1(0.479)。
  8 位读回下 β=0 时角色只有 1/255 量级,量化误差大,要在接近目标的 β 上反推。雾津街头夜里画在原画里的光池处角色仍是剪影(M 0.005–0.018),β 救不了。
