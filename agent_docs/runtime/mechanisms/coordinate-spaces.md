...
> **边界(不是例外,是"它管的不是这个")**:铁律 0 管的是**参与光照计算的量**——
> 法线、灯位、灯向、`N·L`、距离衰减,进着色函数之前必须已在世界空间。
> 而**查表用的索引键**不在此列:载荷若以 q 空间为方向基,查它就必须用世界量的 q 坐标,
> 那是"按这本账的坐标去翻这本账",不是拿 q 量参与计算。
> ⚠ 这条边界是**踩出来的**:曾有人按铁律 0 的字面把查表键也"统一"成世界向量,当天被判为回归并回滚。
> 判一个量属哪边,就问它**是被拿去算光,还是被拿去索引**。细节见
> [character-lighting](character-lighting.md)。
...
2026-08-31 被翻过一次案又回滚 —— **翻案的经过与"别再翻第三次"的理由写在
[character-lighting](character-lighting.md),本卡不复制**。
...
| **光照有没有混空间** | `lc*Light(` 的法线实参必须是**法线图直出的 `n`**(它烘出来就在世界空间);出现 `nW = R·n` 反而是回归 | 机械闸 `worldSpaceShading.test.ts` |
| **probe / 太阳项有没有混空间** | 它们必须收 `nQ = Rᵀ·n`(SH 载荷的方向基在 q) | 同一道机械闸,两头都锁 |
