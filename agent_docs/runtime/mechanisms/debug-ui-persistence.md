...
- **交换档必须是"对讲机",不是状态存档**。四条缺一即静默出错:
  ...
  `lighting.lightFactors` 也属于上述按时段拆合的环境块：角色/粒子各三项倍率与独立 eChroma，F2 改动只走同步槽，
  夜里的倍率必须落在夜变体，不能污染白天基底；正式保存仍由主编辑器统一出口负责。
  主编辑器基底/时段表单都有八项作者控件；F2 显示的是当前实时值。保存验收须走
  F2 发布快照 → 编辑器按时段接收 → Save All → 新 model / 游戏整页重载，不能只断言 uniform 变了。
