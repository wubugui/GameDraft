...
## 进入本模式第一件事：读主线故事进度（必读，阻断式）

进入策划模式后、做任何判断之前，**先完整读一遍** `E:\GameDev\FindingDogStory\故事设计\主线故事进度.md`。它是已落地主线故事的唯一梗概，是"故事现在走到哪儿了"的真相源——本模式里所有关于"这一拍接在哪儿、前面发生过什么、人物此刻是什么处境"的判断都以它为准，不凭记忆、不凭设计稿、不凭对话摘要。

- 没读就动手 = 违规。读完在心里对上主图 `flow_xungou_main` 的 state 顺序再开工。
- **读它不等于可以写它。** 写的规则在第七节，一条不松：没有制作人命令，一个字不许动。

## 进入本模式第二件事：flag 纪律（必读，阻断式，制作人 2026-09-15 定）

**不准自己写 flag；派生 flag 只读；编排一律走叙事状态机。**

- 自己写 flag（`setFlag` / `appendFlag` / `addFlagValue`、往 flag_registry 登记新 flag）默认一律不做；少数实在需要的，第六节汇报里逐条写明为什么状态机表达不了。
- 推进 / 门控 → 信号驱动状态迁移；读取侧用 `{"narrative": 图id, "state": 状态id}` 条件叶。
- "做过没有 / 一次性" → 该段 flow 的状态本身。跨线依赖 → `narrative` 叶直接查对方图的状态。
- 引擎派生的 flag（`has_item_*`、`coins`、`rule_*_acquired`、`quest_*_status`、`archive_*`、`sysnote_*` 等）**只读**：条件里可以读，绝不许写；要改值用对应系统的 action（giveItem、giveCurrency、giveRule、updateQuest…）。

起因：2026-09-15 制作人指出上跑马梁之前那段编排大量用 flag。正文与强制机制 → `agent_docs/content/norms.md` 第 10 条。
**hook 强制**：本技能载入即注入这条；`public/assets/**.json` 的 Edit/Write 写只读 flag 永远被拒，自己写 flag 第一次被拒、实在需要才原样重试；脚本改 JSON 事后追查。条件里读 flag 不拦。
...
- 本次自己写的 flag 逐条列出，每条写明为什么叙事状态机表达不了（没有就写"零自写 flag"）。
- 校验结果：素材审计 + 全量校验是否干净，残留 warning 说明。
- L3 跳过的任务清单及原因/建议。
- 提示用户可在编辑器中打开核对、在游戏中验证效果。

## 七、主线故事进度文档（焊死规则，制作人 2026-09-02 定）

`E:\GameDev\FindingDogStory\故事设计\主线故事进度.md` 是**主线流程的核心跟踪文档**：按主图状态顺序、用制作人口述的川话梗概体，记录已落地的主线故事走到了哪儿。**进入本模式必读**（见开头"第一件事"），读是义务，写是禁区。

1. **绝对禁止擅自写。** 只有制作人明确下令写进这份文档时才动它。本模式做完任何内容——包括接通了新的主线拍——**都不得**自动追加、顺手同步、或在汇报里建议"要不要更新进度文档"。没有命令就当它不存在。
2. **只有梗概，零说明。** 文档里不许出现任何说明性文字：无章节解释、无"本文档是…"、无括号注释、无状态机 id、无文件路径、无待办、无问号、无表格。只有故事。
3. **从落地数据概括，不从设计稿概括。** 写的时候读的是对话图 / 过场 / 叙事状态机里实际存在的内容；设计稿（`Demo制作资料/`、`故事设计/关二狗的故事.md`、配音稿）与落地数据打架时，以落地为准。占位文本（如「吃了一包方便面」）不照抄，按该拍在流程里的实际作用一句带过。
4. **顺序 = 主图状态顺序。** 段落顺序跟 `flow_xungou_main` 的 state 顺序走，不按写作时间。
5. **制作人已写的段落一字不改。** 只在制作人指定的位置追加或替换；未指定的既有段落原样保留。
6. **文体是制作人定的：** 川话梗概、贴身第三人称、短句、不解释、不抒情。落笔前先读文档里已有的段落对口气；过场里没标说话人的台词，先查配音稿核归属再写。

制作人下令核对或更新这份文档时，按 `mainline-story-progress` 技能的流程做（怎么从落地数据追出每一拍、什么算落地、落笔位置、自检与汇报都在那里）。
