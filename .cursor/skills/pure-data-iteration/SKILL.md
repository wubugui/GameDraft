...
1. 根据用户需求，明确需要改动的**数据类型和结构**（例如：任务、遭遇、对话、规矩、物品、商店、演出等）。
2. 查阅 `游戏架构设计文档.md` 中对应系统的数据约定，以及现有数据文件（如 `public/assets/data/*.json`、`public/assets/dialogues/*.ink`、`public/assets/scenes/*.json`）的现有格式与字段。
...
- 只修改纯数据文件（JSON、ink、场景配置等），不修改 TypeScript 或构建配置。
...
2. **引用校验**：所有跨文件或跨条目的 ID 引用是否存在且有效（例如：任务引用的 flag、遭遇引用的规矩、场景引用的热区/NPC、对话引用的 knot 等）。
...
- 简要总结本次改动的文件和内容（例如：改了哪些 JSON/ink、新增/修改了哪些条目）。
...
- 数据格式与字段约定：`游戏架构设计文档.md` 中各系统对 JSON/ink 的说明。
- 现有数据样例与引用方式：`public/assets/data/`、`public/assets/dialogues/`、`public/assets/scenes/` 下的现有文件。
