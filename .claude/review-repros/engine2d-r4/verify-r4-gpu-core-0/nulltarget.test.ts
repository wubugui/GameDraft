...
function mk() {
  ...
  return { rhi, renderer, root };
}

describe('render target falsy', () => {
  it('control: target undefined -> canvas, lastObjectRendered set', () => {
    const { renderer, root } = mk();
    renderer.render({ container: root, target: undefined });
    expect(renderer.lastObjectRendered).toBe(root);
  });
  it('target null throws (Pixi: options.target ||= view.renderTarget)', () => {
    const { renderer, root } = mk();
    let err: unknown = null;
    try { renderer.render({ container: root, target: null as never }); } catch (e) { err = e; }
    console.log('null target error:', String(err));
    expect(err).toBeInstanceOf(TypeError);
    expect(renderer.lastObjectRendered).toBeNull();
    // subsequent canvas render still works (no stuck state)
