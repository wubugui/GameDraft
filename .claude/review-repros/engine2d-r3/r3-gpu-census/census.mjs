...
const PROBE_MASTER = `async () => {
  ...
  const lbl = {}; for (const k in R.texture._managedTextures.items) { const s = R.texture._managedTextures.items[k]; if (!s) continue; const l = String(s.label || 'nolabel').replace(/[0-9]+/g, '#').slice(0, 40); lbl[l] = (lbl[l] || 0) + 1; }
...
async function main() {
  ...
  for (let r = 1; r <= ROUNDS; r++) {
    for (const scene of SCENES) {
      const res = await withTimeout(cmd({ type: 'debugSwitchScene', sceneId: scene }), 60000);
      await sleep(1500);
      // panels
      for (const pn of PANELS) {
        await page.evaluate((pn) => { try { globalThis.__game.stateController.togglePanel(pn); } catch (e) { } }, pn);
        await sleep(250);
        await page.evaluate(() => { try { globalThis.__game.stateController.closeAllPanels(); } catch (e) { } });
        await sleep(100);
      }
      // npc dialogues
      const npcs = await page.evaluate(() => globalThis.__game.sceneManager.getCurrentNpcs().map((n) => n.def?.id ?? n.id).filter(Boolean).slice(0, 3));
      for (const id of npcs) {
        const p = cmd({ type: 'debugInteractNpc', npcId: id });
        await sleep(600);
        for (let k = 0; k < 4; k++) {
          await withTimeout(cmd({ type: 'debugAdvanceDialogue', maxSteps: 20 }), 5000);
          await withTimeout(cmd({ type: 'debugChooseDialogueOption', index: 0 }), 3000);
        }
        await withTimeout(p, 3000);
        await page.keyboard.press('Escape').catch(() => {});
        await sleep(200);
        await page.evaluate(() => { try { globalThis.__game.stateController.closeAllPanels(); } catch (e) { } });
      }
      if (r === 1) console.log('scene', scene, 'switch', res, 'npcs', npcs.join(','));
    }
    samples.push(await sample(`r${r}-active`));
    // idle on the first scene so the old scenes' resources go unused
