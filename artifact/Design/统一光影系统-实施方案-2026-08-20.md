...
>
> **2026-08-30 制作人定调「原画就是最终的光照」**,本文的核心模型
> (`场景 = 原画 × S / S_day` 的整体重打光、角色与场景吃同一个 `S`)**已被否**。
> 现行模型是 **原画 + 加性实体灯**:运行时不重新照亮场景,只把作者摆的灯加上去;
> **「夜」靠换一张夜原画**,不靠调暗天光。统一角色路径被
> `Game.UNIFIED_CHAR_PATH_ENABLED = false` 整条关死。
>
> - 现行机制:`agent_docs/runtime/mechanisms/scene-lighting.md` + `character-lighting.md`
> - 为什么否、继承了哪几条:`agent_docs/runtime/decisions/2026-07-21-scene-radiance-restoration-pipeline.md`
>
> 本文仍然有价值的部分:P0 的世界重建数学、灯的闭式解推导、雾的 σ 模型、
> 去霾与显示变换的实测数字 —— 这些**没有**随模型翻转失效。
> 失效的是"场景怎么被照亮""角色怎么并入"这两章。

# 统一光影系统 · 实施方案

- 日期：2026-08-20（P3–P5 于 08-21 落地）｜ 分支：`lighting-rebuild`
- 状态：**已被 2026-08-30 的「原画 + 加性实体灯」取代**（见文首横幅）。当时的结论是 P0–P6 已实现并验收；其中 P1 场景自然光、P3 角色并入、P5 GI 三章现已停用。
