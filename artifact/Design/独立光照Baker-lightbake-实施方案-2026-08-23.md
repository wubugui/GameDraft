...
### base 色度反噬与方案 A:E 色度钳(2026-08-25)

诊断(teahouse 灯角,证据页):`base = 原画⊘E` 的**逐通道除法**把 E 的
色度以倒数写进 base —— E 的 R 色度 vs base 的 R 色度逐像素相关 **−0.807**
(E 橙 ⇒ base 青蓝互补)。gi=1 时被 gi·E 精确乘回、恒等不受影响;重打光
引入色度 ≠ 烘焙 E 的项(中性天光/E_环境)时互补色显形。次因:E 估计的
灯晕落差与原画画的落差不逐像素重合 ⇒ 彩色比值环。

**方案 A(制作人拍板试做)**:`gather.chroma_clamp_e` —— E 色度向中性钳
(亮度**精确保持**,c=E/lum 钳 [1/(1+τ),1+τ] 后按亮度重归一),做在
`recombine_sky` 内 compose 之后、gain 之前(唯一编排 ⇒ GUI/CLI/#12(c)
自动一致);存储的 E 只有一份 ⇒ base 除它、运行时 gi 乘它天然双侧一致,
**gi=1 恒等逐字节不动**。旋钮:`--e-chroma-clamp` / bakeParams
`eChromaClamp` / GUI 重估级实调(即调即回帧),缺省关。灯色大头随之
...
### GUI v4(2026-08-25,制作人实用反馈六条全落)

①**场景浏览器**:`list_bakeable()` 全场景下拉,即开即烘首帧 —— 打开场景用
**该场景 bakeParams 三级决议值**(不带上个场景的面板残留),面板随 ctx 回填;
②**base 彩色通道**(Reinhard 显示;亮度/色度保留);③**AO 参与着色**:
`preview.ambient_env` = §6.1 的 E_环境(环境色 RGB+强度旋钮,AO 的消费处
终于可见)+ **体矩口径最终渲染通道**(`preview.volume_sky_at_surface` 三线性
重建 (a₀,a₁) 替换逐像素矩着色 = 角色受光一致性直接目视,#5 的口径);
④**运行时天空全参数实时可调**(intensity/profile/kelvin/horizon*/glow*/
ground*/太阳 el·az;预设与「场景自身 lighting.sky」降级为初始化器;
sky_response 按参数键缓存,150ms 合并);第三个存回按钮写 `lighting.sky`
(input.save_runtime_sky,统一写盘出口 —— 与 bakeSky/bakeParams 三键分明);
⑤**真进度条**(progress_cb → QProgressBar,阶段名 + i/n);⑥**单版参数
纪律**:打开场景 = 场景决议这一版,「重烘/全量 bake」= 当前面板这一版,
一次只烘一版,CLI 行如实回显 —— 不存在多套参数对比烘焙。
...
### §15 追记:探针球/立绘自由移动(2026-08-26)

制作人:「probe球和角色这种都要能够我自由移动」。视口交互升级:按在已画出的
探针球/立绘上=**抓起来拖**(带按点偏移不跳变,~33fps 节流,松手补帧),
抓取优先于一切放置模式——移动现有对象不再需要切勾;空白处点击才走放置
(放灯>放角色>放探针),放下即刻可拖。命中区由 overlay 每帧回填
(球=圆、立绘=合成矩形)。冒烟 +1(抓/拖/优先级/放置即抓四断言),
全套 171+1。


### §15 追记:立绘镜像三审修复轮(2026-08-26)

立绘二审(独立复核 + 15 突变)裁决:天光/光源链逐式且被钉,但几何两处
实错、图集口径一处实错、AO/GI 钳位次序反,10/15 突变存活。全单修复:

- **P0-1 高度少除 cosT**:运行时世界竖直位移 = 屏幕 Δy/(cosT·ppu)
  (SpriteEntity 的屏幕高与 qu_h·ppu 逐位相等已核对),θ=32° 的场景此前
  整个矮 15.2%、头部 q.z 少退 14.2wu(够翻转贴阈阴影)。修:
  `hy = Δrow/(R[1,1]·ppu)`(cosT 就是 R[1,1],同一份 R 不开第二真相源)。
- **P0-2 彩色图集双重反预乘**:浏览器对彩色图集解码期预乘,shader 的
  `/max(a,1e-4)` 是**还原**;PIL 读的本来就是直通 α ⇒ 不再除
  (player 图集 15.4% 可见像素部分 α,此前边缘至多 1.8× 亮边)。缩放
  口径同修:彩色在**预乘域**双线性缩后除回、法线直通,全程 float(PIL
  'F' 模式),不再有 uint8 往返。
- **P1-1 宽走 worldWidth**:两轴独立系数(player 0.988 vs 格子比 1.073,
  差 8.65%,横向 q 也跟着撑宽)。anim.json worldWidth 进 CharSprite。
- **P1-2 AO/GI 逐角点 max(a0+a1·N,0) 后插值**(运行时 ucGridFetch 口径;
  天穹通道保持系数插值后求值)——方向性 GI 的背光侧不再被邻角正值抵消。
  **P2-1** ao 入参不再预钳 ≤1(1.2 天花板恢复可达)。
- 其余:discard 阈对齐 0.03;合成底行画在脚点行(y1=iy+1);锚一致取
  cw//2;charRefIntensity 的 ??0 语义;scale_mul=depthScaleFactor 旋钮 +
  状态/帧选择进 GUI;探针读数走 note2 专线(立绘报错不再被当场覆盖);
  char_on 只在 final 系通道吞点击(a0 通道放探针不再被饿死);立绘侧
  notes(截断/未知 kind/脚点疑似墙面)出声。
- **测试**:夹具三戒(ao≠1、α≠全 1、ne.a≠恒 0)+ 每个存活突变一根钉:
  cosT 高度(体 GI 沿 y 三层的顶行读数)、直通 α(半透明逐位同 + discard)、
  worldWidth 形状、角点钳位次序(0.25 vs 0.1 的中点甄别)、ao 上界、
  cap0 地板(朝下法线 + 微 t₀ 全链复算)、b 地板(正面光观测)、bulge
  翻号(z 两层)、mirror 只翻 x(斜光精确值)、环境曲线、cut_gate、
  形体 AO smoothstep、批量 march 与标量版 40 点逐位奇偶校验(构造性
  两组:必穿墙/已越墙 + 单列薄墙的**march 网格粒度甄别**——步数减半只缩粒度,墙在前半段会滑过,细步网格恰踩的 col 是唯一裁判)。**三审突变 16/16 落盘验杀全红**。全套 184+1。


### §15 追记:探针逐像素位形 + 实体缩放(2026-08-26)

制作人抓到实差:「真的是逐像素取伪世界空间位置着色的么?我咋感觉一直取的
落点」。坦白账:**立绘一直是真逐像素**;探针球此前是「球心单点体采样 +
逐像素法线」,灯也拿球心位置算 —— 贴灯放球看不到球面梯度,手感正确。

- 探针改**真逐像素位形**:球面每像素 P = center + r_q·N(世界),进体采样
  与灯计算;球心不再是唯一位置。
- 收敛出两个**实体口径唯一实现**(立绘/探针共用,不再各抄):
  `character.sample_entity_volume`(ucSkyAt:天穹系数插值后求值、AO/GI
  逐角点 max0 后插值)与 `lights.eval_entity_lights`(太阳逐像素 march
  48/3.5 不过 R、灯逐像素 ucLightVisibility 批量、无 cut 早退、24 截断);
  `eval_probe_lights` 降为单点位形薄包装。
- `shade_probe_ball` 签名改 (ctx, center, …, radius_q, lights)——体与灯
  全部内算;comps 新增逐像素 `qz`,探针 X-Ray 升级为逐像素深度比。
- **缩放**:探针「直径(wu)」旋钮(缺省 150)+ **Ctrl+拖=缩放**;立绘
  Ctrl+拖调 dsf(150wu=1×);Shift+拖=高度不变。地面圈随直径走。
- 新钉:球面左右吃不同体数据(GI 沿 x 两层 ⇒ 右>左 1.15×,旧实现恒等)、
  贴灯球面梯度(左>右 1.15×)、中心像素定量(fr=0.5 ⇒ g=0.5)、直径→
  命中半径、Ctrl 拖调直径。全套 189+1。


### §15 追记:实体锚从像素改世界坐标(2026-08-26)

制作人报「落点一被遮挡,半空的球立即熄灯,且突变」。竖直剖线取证
(雾津街头):点到的「屋顶」在伪世界重建里是壳 —— 该像素反投影为
y=−1wu、z=+0.44q(深巷底),旁边街面像素是 y=168wu、z=−0.99q。
「沿 up 抬 196wu」抬的是深巷峡谷上空,体数据 sky_a0=0.05~0.21,球暗是
**场的真实值**;「突变」= 拖动时锚每帧取光标下像素表面,街面→屋顶壳的
一瞬实体在伪世界瞬移 1.4q。不是着色对遮挡的依赖 —— 是**像素锚范式**的根病。

修法(治本):实体锚改**世界坐标**(`_probe_w`/`_char_w` 为唯一真相):

- 放置那一下仍取表面点(点哪儿摆哪儿);
- **拖动 = 在初始锚点水平面内解算**:视口位移 → 平面内 (Δx,Δz) 的 2×2
  线性系(`_drag_plane_delta`,近奇异即 R≈I 正交视角时退化为只动 x);
  实体连续滑动、永不瞬移,光照只取决于世界位置,与任何像素遮挡无关;
- 像素(`_probe_px`/`_char_foot`)降为派生量(投影回像素,供读数与
  测试直插兼容,直插时按需物化世界锚);
- 同场景重烘**保留**世界锚(锚不随烘焙失效);换场景清空。

新钉:R=I 下拖动 x 跟手、y(深度轴)锁定、**世界 z 逐位不动**(无瞬移),
球与立绘各一组。全套 189+1。

### §15 追记:GI 体 SH-L2 升级与收敛审计(2026-08-26)

制作人立法:体 GI 在分辨率→∞ 时应无限接近逐像素 E,并要求自查商业引擎。
判定:E(N)=a0+a1·N 是钳位余弦的 L1 展开(¼+½cos,与 sc3SHTransfer 同核),
核能量份额 L0≈25% / L1≈50% / L2≈23.4% / 余≈1.6% —— L1 有不随分辨率消失的
~23% 核截断底(外带地平线下泄漏)。UE Volumetric Lightmap 与 Unity Light
Probes 用 3 阶球谐、DDGI 用八面体辐照+可见性,正是为此。故升 SH-L2:

- 烘焙:gi_sh(n,9,3) 与 (a0,a1) 同批样本投影(BSDF 命中 / NEE 光源样本 /
  天空逃逸各按自己方向);烘内构造性自检 c00·Y00≡a0(代数恒等)。
- 求值:E(N)/π = Σ c·k·Y(N),k=(1, 2/3, 1/4);逐角点 max0 后插值次序不变;
  旧 ctx 回落 L1。场景表面重建 volume_gi_at_surface 与实体同一采样器。
- GUI:e_givol(场景/实体同吃 GI volume,同一个量并排)+ final_givol
  (base·E_GIvol)双通道,note 常驻 vs E间接·gain 的中位/p95 读数。
- packed 8-bit 载荷仍 L1:L2 量化编码是 P7 接线时的契约设计,记档不抢跑。

收敛审计(雾津街头,work512,阈=亮度前 80%):

| 配置 | L2 med/p95 | L1 med/p95 |
|---|---|---|
| 密度2(52×14×48,spp96/ref24) | 7.2% / 35.7% | — |
| 密度4(92×25×84,spp96/ref24) | 7.0% / 38.2% | — |
| 密度8(209×58×190=2.3M,spp128/ref24) | 6.4% / 40.7% | 9.9% / 49.8% |
| 密度12(3.95M,spp128/ref24) | 6.5% / 44.6% | 10.0% / 53.1% |
| 密度4,双侧高 spp(vol512/ref256) | **5.4% / 35.2%** | 8.6% / 43.1% |

结论三段:① L2 对 L1 的收益稳定 ~35%(同烘同评,隔离干净);② 噪声占
中位 ~1.6pp(高 spp 后 7.0→5.4),其余 ~5% 是**结构性底**;③ 热图显示
p95 集中在檐下/背光立面与屋脊棱线 —— 半球裁剪 + 贴墙格点 dilation 混插的
probe-vs-surface 经典签名,开阔面 <10%。首次审计曾被 vol_max_cells 缺省
上限暗中钳住(密度 8 与 4 同网格)—— 扫密度必须显式抬上限。
未来杠杆(P7+ 候选,记档不实施):DDGI 式逐探针可见性权重治檐下带;
表面重建时按 validity 掩码降权被埋角点。

### §15 追记:GI 体「乱造轮子」质疑的证伪实验(2026-08-26)

制作人质疑:GI 体走样 = 没复用逐像素 E 的那套 tracer/MC;判据 = GI 体对
逐像素 E 的关系应与 AO 体对逐像素 AO 相同。逐项核验:

1. 共享面(逐行):GI 趟与逐像素 E 同一 trace(march 注册辐射)、同一
   hdr_gained、同一 sky_of_gained、同一 NEE+MIS(壳箱弦,同一实现)、
   同一 clamp、同一 QMC 族;天穹矩干脆是同一个函数(sky_moments)。
   唯一必然差:体格点不知道未来法线 ⇒ 均匀球面 + 角向基(AO 体同款)。
2. 嫌疑犯证伪(密度4/vol256/ref spp96):validity 加权 med 5.7→5.8%、
   法线偏移半格 5.7→6.2%(p95 35.3→33.1%)—— dilation 混插与贴面采样
   都不是主因,不改重建。
3. 判据成立:AO 体重建 vs 逐像素 AO 的 med=6.7% / p95=39.8%(L1);
   GI-L2 为 5.7% / 35.3% —— GI 与 AO 同量级、且更贴(L2 之功)。
   「中面板的块状」= 三线性 C0 重建在大平面上的固有长相(运行时
   ucGridFetch 同为三线性,镜像不换插值)。
4. 残差三源(记档):角向基截断(AO 仍 L1,升级候选)、几何棱线处的
   格内空间频率(随密度收敛)、参考侧 spp/平滑差。

### §15 追记:「歪七扭八走样」双假设证伪 + 体AO对照通道(2026-08-26)

制作人在 GUI(首帧缺省:密度3,格≈50wu)看到体GI重建的流状走样,并称
「AO 重建没有」。两轮实验:

1. 我的 L2 振铃假设**证伪**:同烘同区放大对比,L1 面板与 L2 形态相同;
   SH 加窗 w2=0.7/0.5 无肉眼改善且数字变差(med 6.6%→7.0/8.0%)——
   不加窗,不为好看数字瞎动。
2. 「AO 没有走样」的真相:制作人对比的是**逐像素 AO 通道**(逐像素烘的
   当然干净);GUI 此前没有「从 AO 体重建」的通道。渲出来后:AO 体重建
   的走样形态与 GI **一模一样**——这是「矩体+三线性+逐像素法线求值」在
   该密度下的固有长相,非 GI 特有,GI-L2 数值上还是几个场里最贴的。
