...
射线离开画幅之后画面里没有任何信息，按逃逸取天空辐射，**零自由参数**。
试过「侧边/底边出画一律当挡住」：室内修好了但户外变差，开阔处自检从 1.000 掉到 0.94——
一刀切的启发式两头不讨好，全部否掉。现役 `bake_gather` / `mc_sky_moments` 与已提交的
`trace_rays` 都是这个语义，别再改。
...
上面的推进循环按「**一根射线走完一生**」执行，不按「全体射线同走一步」执行。
前者是 pbrt / Arnold / Cycles / Embree 所有 CPU 渲染器的共同结构：并行单元是
光线（或 tile / bucket）的**完整生命周期**，每线程只读场景、只写自己那段结果，
零共享写、零同步。后者（wavefront：整批推一步 → 压缩存活 → 再推一步）是
Laine 2013 为治 GPU warp divergence 发明的，不该往 CPU 搬——CPU 的乱序执行
本来就吃得掉控制流发散，而 wavefront 每步把全批光线状态刷一遍内存的代价
在 CPU 上是纯亏。

numpy 向量化的 march **就是一个 CPU 上的 wavefront**，它跑不满有三条实测病灶
（数字全在 §15「tracer 执行模型实测」）：

1. **内存墙**：每步物化 ~10 个全批 float32 临时数组（`zq/sx/sy/keep/xi/yi/pen/hit`…），
   590k 射线时没有一个装得进 L2，每步全程 L3/DRAM 往返。逐射线执行时 `(o,d,t)`
   全程在寄存器，唯一访存是 `depth[yi,xi]` 一次 gather，算术强度差两个数量级。
   旁证：仅把 numpy 版按 64k 射线分块（工作集贴住缓存）就白赚 **1.7×**。
2. **GIL**：每轮 while 十几个 numpy 调度点，全屏一趟 2636 轮 ≈ 4 万次解释器往返。
   给它套 ThreadPool 实测 12 线程 **1.11×**——不是没写对，是这个模型在 CPython
   里线程化不了。多进程也只推到 ~6.9× 撞墙，还得处理跨进程共享 depth 与 RNG。
3. **长尾**：整批同步意味着 **56% 的轮数在伺候不到 5% 还活着的射线**
   （平均 316 步/射线，最长 2636 轮）。逐射线执行没有「轮」的概念，长尾自动消失。
...
- **方向采样、RNG 全部留在 numpy 主线程**（`sampling.py` 本来就在核外，
  见「职责边界」）。RNG 流与分块、线程数、后端完全无关；生成 590k 个方向
  是毫秒级，没有任何理由为并行去动随机数。
- **中间量 dtype（float32）、取整（`rint`）、比较顺序与参考实现逐字对齐**，
  否则契约测试 6 过不了。
- **并行是 `trace.py` 的内部实现细节**：`trace()` 签名不变、没有 threads 参数；
  线程数由 CLI `--threads` 经 `numba.set_num_threads` 全局设一次（0 = 全部逻辑核）。

**参考实现 `_march_reference` 保留，不是死代码**：numpy 分块 wavefront
（`TRACE_REF_CHUNK = 65536`，实测最优；整批慢 1.7×，8k 块也回落，§15）。三个职责：
① numba 装不上时的降级路径——降级必须**大声**：控制台与 report 标明 backend
（不进 meta：两个后端产物逐位相同，meta 记后端反而让跨机器 diff 变脏）；
② 契约测试 6 的语义金标准；③ 未来改 tracer 时的交叉验证基线。
参考实现与编译核是**同一语义的两个表达**：改判据必须两边一起改，
契约测试 6 保证它们不漂移。
...
6. **编译核 ≡ 参考实现**：同一批起点、方向、`max_distance`，`_march_kernel` 与
   `_march_reference` 的 `escaped / hit_yx / t_hit` **逐位相同**
   （原型已验：590k 随机射线 0 mismatch，§15）。改判据必须两边一起改——
   参考实现是语义规格，不是化石。
...
tools/lightbake/
    ...
    trace.py        ★★ 独立通用组件：DepthField / trace() / buried() / TraceResult
                    ...
                    内部两个后端（§5.4「执行模型」）：_march_kernel（numba 逐射线
                    编译核，生产路径）/ _march_reference（numpy 分块 wavefront，
...
| `TRACE_REF_CHUNK` | 65536 | 参考实现（numpy 后端）的分块射线数：工作集贴住缓存，实测比整批快 1.7×、比 8k 块快 1.8×，逐位不变（§15）。编译核不用它 |
...
| 多进程（numpy 后端） | 4 进程有效吞吐 ×6.8；12 进程 ×6.9 —— 撞墙 |
| 长尾 | while 共 2636 轮；后 **56%** 的轮数只剩 <5% 存活射线 |

#### 逐射线编译核（numba prange 原型，判据逐字同参考实现）
